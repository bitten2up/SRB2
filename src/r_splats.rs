...
extern "C" {
    ...
    static mut ds_translation: *mut lighttable_t;
    ...
    fn Patch_GenerateFlat(patch: *mut patch_t, flags: pictureflags_t);
    fn R_CalculateSlopeVectors();
    fn R_SetScaledSlopePlane(
        slope: *mut pslope_t,
        xpos: fixed_t,
        ypos: fixed_t,
        zpos: fixed_t,
    ...
    static mut cachedystep: [fixed_t; 1200];
    static mut cachedxstep: [fixed_t; 1200];
    static mut cacheddistance: [fixed_t; 1200];
    static mut cachedheight: [fixed_t; 1200];
    fn R_GetSpriteTranslation(vis: *mut vissprite_t) -> *mut uint8_t;
    fn R_SetTiltedSpan(span: int32_t);
...
pub type renderflags_t = libc::c_uint;
pub const RF_DROPSHADOW: renderflags_t = 49664;
pub const RF_SHADOWEFFECTS: renderflags_t = 32768;
pub const RF_SHADOWDRAW: renderflags_t = 16384;
pub const RF_FLOORSPRITE: renderflags_t = 8192;
pub const RF_PAPERSPRITE: renderflags_t = 4096;
pub const RF_SPRITETYPEMASK: renderflags_t = 12288;
pub const RF_NOCOLORMAPS: renderflags_t = 1024;
pub const RF_SEMIBRIGHT: renderflags_t = 768;
pub const RF_FULLDARK: renderflags_t = 512;
pub const RF_FULLBRIGHT: renderflags_t = 256;
pub const RF_BRIGHTMASK: renderflags_t = 768;
pub const RF_NOSPLATROLLANGLE: renderflags_t = 128;
pub const RF_NOSPLATBILLBOARD: renderflags_t = 64;
pub const RF_OBJECTSLOPESPLAT: renderflags_t = 32;
pub const RF_SLOPESPLAT: renderflags_t = 16;
pub const RF_SPLATMASK: renderflags_t = 240;
pub const RF_FLIPOFFSETS: renderflags_t = 8;
pub const RF_ABSOLUTEOFFSETS: renderflags_t = 4;
pub const RF_VERTICALFLIP: renderflags_t = 2;
pub const RF_HORIZONTALFLIP: renderflags_t = 1;
pub type C2RustUnnamed_2 = libc::c_uint;
pub const SRF_NONE: C2RustUnnamed_2 = 255;
pub const SRF_2D: C2RustUnnamed_2 = 12;
pub const SRF_RIGHT: C2RustUnnamed_2 = 8;
pub const SRF_LEFT: C2RustUnnamed_2 = 4;
pub const SRF_3DMASK: C2RustUnnamed_2 = 3;
pub const SRF_3DGE: C2RustUnnamed_2 = 2;
pub const SRF_3D: C2RustUnnamed_2 = 1;
pub const SRF_SINGLE: C2RustUnnamed_2 = 0;
#[derive(Copy, Clone)]
#[repr(C)]
...
pub struct vissprite_s {
    ...
    pub viewpoint: C2RustUnnamed_4,
    pub shear: C2RustUnnamed_3,
...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct floorsplat_s {
    pub pic: *mut uint16_t,
    pub width: int32_t,
    pub height: int32_t,
    pub scale: fixed_t,
    pub xscale: fixed_t,
    ...
    pub angle: angle_t,
    pub slope: *mut pslope_t,
    pub verts: [vector3_t; 4],
    pub x: fixed_t,
    pub y: fixed_t,
    pub z: fixed_t,
    pub mobj: *mut mobj_t,
}
pub type floorsplat_t = floorsplat_s;
#[inline(always)]
...
#[no_mangle]
pub static mut prastertab: *mut rastery_s = 0 as *const rastery_s as *mut rastery_s;
static mut rastertab: [rastery_s; 1200] = [rastery_s {
    minx: 0,
    maxx: 0,
    tx1: 0,
    ty1: 0,
...
unsafe extern "C" fn rasterize_segment_tex(
    mut x1: int32_t,
    mut y1: int32_t,
    mut x2: int32_t,
    mut y2: int32_t,
    ...
    let mut xe: fixed_t = 0;
    let mut count: fixed_t = 0;
    let mut dx0: fixed_t = 0;
    let mut dx1: fixed_t = 0;
    if y1 == y2 {
        return;
    }
    if y2 > y1 {
        count = y2 - y1 + 1 as libc::c_int;
        dx0 = FixedDiv(x2 - x1 << 16 as libc::c_int, count << 16 as libc::c_int);
        dx1 = FixedDiv(tv2 - tv1 << 16 as libc::c_int, count << 16 as libc::c_int);
        xs = x1 << 16 as libc::c_int;
        xe = tv1 << 16 as libc::c_int;
        tc <<= 16 as libc::c_int;
        if dir == 0 as libc::c_int {
            loop {
                rastertab[y1 as usize].maxx = xs;
                rastertab[y1 as usize].tx2 = xe;
                rastertab[y1 as usize].ty2 = tc;
                xs += dx0;
                xe += dx1;
                y1 += 1;
                y1;
            ...
            loop {
                rastertab[y1 as usize].maxx = xs;
                rastertab[y1 as usize].tx2 = tc;
                rastertab[y1 as usize].ty2 = xe;
                xs += dx0;
                xe += dx1;
                y1 += 1;
                y1;
        ...
        count = y1 - y2 + 1 as libc::c_int;
        dx0 = FixedDiv(x1 - x2 << 16 as libc::c_int, count << 16 as libc::c_int);
        dx1 = FixedDiv(tv1 - tv2 << 16 as libc::c_int, count << 16 as libc::c_int);
        xs = x2 << 16 as libc::c_int;
        xe = tv2 << 16 as libc::c_int;
        tc <<= 16 as libc::c_int;
        if dir == 0 as libc::c_int {
            loop {
                rastertab[y2 as usize].minx = xs;
                rastertab[y2 as usize].tx1 = xe;
                rastertab[y2 as usize].ty1 = tc;
                xs += dx0;
                xe += dx1;
                y2 += 1;
                y2;
            ...
            loop {
                rastertab[y2 as usize].minx = xs;
                rastertab[y2 as usize].tx1 = tc;
                rastertab[y2 as usize].ty1 = xe;
                xs += dx0;
                xe += dx1;
                y2 += 1;
                y2;
...
#[no_mangle]
pub unsafe extern "C" fn R_DrawFloorSplat(mut spr: *mut vissprite_t) {
    let mut splat: floorsplat_t = floorsplat_s {
        pic: 0 as *mut uint16_t,
        width: 0,
        height: 0,
        scale: 0,
        xscale: 0,
        yscale: 0,
        angle: 0,
        slope: 0 as *mut pslope_t,
        verts: [vector3_t { x: 0, y: 0, z: 0 }; 4],
        x: 0,
        y: 0,
        z: 0,
        mobj: 0 as *mut mobj_t,
    };
    let mut mobj: *mut mobj_t = (*spr).mobj;
    let mut tr_x: fixed_t = 0;
    let mut tr_y: fixed_t = 0;
    let mut rot_x: fixed_t = 0;
    let mut rot_y: fixed_t = 0;
    let mut rot_z: fixed_t = 0;
    let mut v3d: *mut vector3_t = 0 as *mut vector3_t;
    let mut v2d: [vector2_t; 4] = [vector2_t { x: 0, y: 0 }; 4];
    let mut rotated: [vector2_t; 4] = [vector2_t { x: 0, y: 0 }; 4];
    let mut x: fixed_t = 0;
    let mut y: fixed_t = 0;
    let mut w: fixed_t = 0;
    let mut h: fixed_t = 0;
    let mut angle: angle_t = 0;
    let mut splatangle: angle_t = 0;
    let mut ca: fixed_t = 0;
    let mut sa: fixed_t = 0;
    let mut xscale: fixed_t = 0;
    let mut yscale: fixed_t = 0;
    let mut xoffset: fixed_t = 0;
    let mut yoffset: fixed_t = 0;
    let mut leftoffset: fixed_t = 0;
    let mut topoffset: fixed_t = 0;
    let mut i: int32_t = 0;
    let mut hflip: boolean = ((*spr).xiscale < 0 as libc::c_int) as libc::c_int;
    let mut vflip: boolean = (*spr).cut as libc::c_int & SC_VFLIP as libc::c_int;
    let mut flipflags: uint8_t = 0 as libc::c_int as uint8_t;
    let mut renderflags: renderflags_t = (*spr).renderflags as renderflags_t;
    if hflip != 0 {
        flipflags = (flipflags as libc::c_int | PICFLAGS_XFLIP as libc::c_int)
            as uint8_t;
    }
    if vflip != 0 {
        flipflags = (flipflags as libc::c_int | PICFLAGS_YFLIP as libc::c_int)
            as uint8_t;
    }
    if mobj.is_null() || P_MobjWasRemoved(mobj) != 0 {
        return;
    }
    Patch_GenerateFlat((*spr).patch, flipflags as pictureflags_t);
    splat.pic = (*(*spr).patch).flats[flipflags as usize] as *mut uint16_t;
    if (splat.pic).is_null() {
        return;
    }
    splat.mobj = mobj;
    splat.width = (*(*spr).patch).width as int32_t;
    splat.height = (*(*spr).patch).height as int32_t;
    splat.scale = (*mobj).scale;
    if !((*mobj).skin).is_null()
        && (*((*mobj).skin as *mut skin_t)).flags as libc::c_uint
            & SF_HIRES as libc::c_int as libc::c_uint != 0
    {
        splat
            .scale = FixedMul(
            splat.scale,
            (*((*mobj).skin as *mut skin_t)).highresscale,
        );
    }
    if (*spr).rotateflags as libc::c_int & SRF_3D as libc::c_int != 0
        || renderflags as libc::c_uint
            & RF_NOSPLATBILLBOARD as libc::c_int as libc::c_uint != 0
    {
        splatangle = (*mobj).angle;
    } else {
        splatangle = (*spr).viewpoint.angle;
    }
    if (*spr).cut as libc::c_int & SC_ISROTATED as libc::c_int == 0 {
        splatangle = splatangle.wrapping_add((*mobj).spriteroll);
    }
    splat.angle = splatangle.wrapping_neg();
    splat.angle = (splat.angle).wrapping_add(0x40000000 as libc::c_int as angle_t);
    topoffset = (*spr).spriteyoffset;
    leftoffset = (*spr).spritexoffset;
    if hflip != 0 {
        leftoffset = splat.width * ((1 as libc::c_int) << 16 as libc::c_int)
            - leftoffset;
    }
    ...
    splat.y = y;
    splat.z = (*spr).pz;
    splat.slope = 0 as *mut pslope_t;
    splat.verts[0 as libc::c_int as usize].x = w - xoffset;
    splat.verts[0 as libc::c_int as usize].y = yoffset;
    splat.verts[1 as libc::c_int as usize].x = -xoffset;
    splat.verts[1 as libc::c_int as usize].y = yoffset;
    splat.verts[2 as libc::c_int as usize].x = -xoffset;
    splat.verts[2 as libc::c_int as usize].y = -h + yoffset;
    splat.verts[3 as libc::c_int as usize].x = w - xoffset;
    splat.verts[3 as libc::c_int as usize].y = -h + yoffset;
    angle = (splat.angle).wrapping_neg() >> 19 as libc::c_int;
    ca = *finecosine.offset(angle as isize) >> 16 as libc::c_int - 16 as libc::c_int;
    sa = finesine[angle as usize] >> 16 as libc::c_int - 16 as libc::c_int;
    i = 0 as libc::c_int;
    while i < 4 as libc::c_int {
        rotated[i as usize]
            .x = FixedMul(splat.verts[i as usize].x, ca)
            - FixedMul(splat.verts[i as usize].y, sa);
        rotated[i as usize]
            .y = FixedMul(splat.verts[i as usize].x, sa)
            + FixedMul(splat.verts[i as usize].y, ca);
        i += 1;
        i;
    }
    if renderflags as libc::c_uint
        & (RF_SLOPESPLAT as libc::c_int | RF_OBJECTSLOPESPLAT as libc::c_int)
            as libc::c_uint != 0
    {
        let mut standingslope: *mut pslope_t = (*mobj).standingslope;
        if renderflags as libc::c_uint & RF_SLOPESPLAT as libc::c_int as libc::c_uint
            != 0
        {
            splat.slope = (*mobj).floorspriteslope;
        }
        if !standingslope.is_null()
            && renderflags as libc::c_uint
                & RF_OBJECTSLOPESPLAT as libc::c_int as libc::c_uint != 0
        {
    ...
    i = 0 as libc::c_int;
    while i < 4 as libc::c_int {
        tr_x = rotated[i as usize].x + (*mobj).x;
        tr_y = rotated[i as usize].y + (*mobj).y;
        if !(splat.slope).is_null() {
            rot_z = P_GetSlopeZAt(splat.slope, tr_x, tr_y);
            splat.verts[i as usize].z = rot_z;
        } else {
            splat.verts[i as usize].z = splat.z;
        }
        splat.verts[i as usize].x = tr_x;
        splat.verts[i as usize].y = tr_y;
        i += 1;
        i;
    }
    angle = (*spr).viewpoint.angle >> 19 as libc::c_int;
    ca = *finecosine.offset(angle as isize) >> 16 as libc::c_int - 16 as libc::c_int;
    sa = finesine[angle as usize] >> 16 as libc::c_int - 16 as libc::c_int;
    i = 0 as libc::c_int;
    while i < 4 as libc::c_int {
        v3d = &mut *(splat.verts).as_mut_ptr().offset(i as isize) as *mut vector3_t;
        tr_x = (*v3d).x - (*spr).viewpoint.x;
        tr_y = (*v3d).y - (*spr).viewpoint.y;
        rot_x = FixedMul(tr_x - ((*mobj).x - x), sa)
            - FixedMul(tr_y - ((*mobj).y - y), ca);
        rot_y = FixedMul(tr_x - ((*mobj).x - x), ca)
            + FixedMul(tr_y - ((*mobj).y - y), sa);
        rot_z = (*v3d).z - (*spr).viewpoint.z;
        if rot_y < (1 as libc::c_int) << 16 as libc::c_int {
            return;
        }
        xscale = FixedDiv(projection, rot_y);
        yscale = -FixedDiv(projectiony, rot_y);
        v2d[i as usize].x = centerxfrac + FixedMul(rot_x, xscale) >> 16 as libc::c_int;
        v2d[i as usize].y = centeryfrac + FixedMul(rot_z, yscale) >> 16 as libc::c_int;
        i += 1;
        i;
    }
    R_RasterizeFloorSplat(&mut splat, v2d.as_mut_ptr(), spr);
}
unsafe extern "C" fn R_RasterizeFloorSplat(
    mut pSplat: *mut floorsplat_t,
    mut verts: *mut vector2_t,
    mut vis: *mut vissprite_t,
) {
    let mut miny: int32_t = viewheight + 1 as libc::c_int;
    let mut maxy: int32_t = 0 as libc::c_int;
    let mut y: int32_t = 0;
    let mut x1: int32_t = 0;
    let mut ry1: int32_t = 0;
    let mut x2: int32_t = 0;
    let mut y2: int32_t = 0;
    let mut i: int32_t = 0;
    let mut offsetx: fixed_t = 0 as libc::c_int;
    let mut offsety: fixed_t = 0 as libc::c_int;
    let mut planeheight: fixed_t = 0 as libc::c_int;
    let mut step: fixed_t = 0;
    let mut spanfunctype: libc::c_int = 0;
    ds_source = (*pSplat).pic as *mut uint8_t;
    ds_flatwidth = (*pSplat).width as uint16_t;
    ds_flatheight = (*pSplat).height as uint16_t;
    ...
    if R_CheckSolidColorFlat() != 0 {
        ...
        R_SetFlatVars(
            (ds_flatwidth as libc::c_int * ds_flatheight as libc::c_int) as size_t,
        );
        ds_powersoftwo = true_0 as libc::c_int;
    }
    if !((*pSplat).slope).is_null() {
        R_SetTiltedSpan(0 as libc::c_int);
        R_SetScaledSlopePlane(
            ...
            (*pSplat).xscale,
            (*pSplat).yscale,
            -(*pSplat).verts[0 as libc::c_int as usize].x,
            (*pSplat).verts[0 as libc::c_int as usize].y,
            (*vis).viewpoint.angle,
            (*pSplat).angle,
        );
        R_CalculateSlopeVectors();
    } else if ds_solidcolor == 0 {
        ...
        if (*pSplat).angle != 0 {
            ...
            let mut a: fixed_t = -(*pSplat).verts[0 as libc::c_int as usize].x
                + (*vis).viewpoint.x;
            let mut b: fixed_t = -(*pSplat).verts[0 as libc::c_int as usize].y
                + (*vis).viewpoint.y;
            let mut angle: angle_t = (*pSplat).angle >> 19 as libc::c_int;
            offsetx = FixedMul(
                a,
                *finecosine.offset(angle as isize)
            ...
            offsetx = (*vis).viewpoint.x - (*pSplat).verts[0 as libc::c_int as usize].x;
            offsety = (*pSplat).verts[0 as libc::c_int as usize].y - (*vis).viewpoint.y;
        }
    }
    ds_colormap = (*vis).colormap;
    ds_translation = R_GetSpriteTranslation(vis);
    if ds_translation.is_null() {
        ds_translation = colormaps;
    }
    if !((*vis).extra_colormap).is_null() {
        if ds_colormap.is_null() {
            ds_colormap = (*(*vis).extra_colormap).colormap;
        } else {
            ds_colormap = &mut *((*(*vis).extra_colormap).colormap)
    ...
    if ds_solidcolor != 0 {
        let mut px: uint16_t = *(ds_source as *mut uint16_t);
        if px as libc::c_int & 0xff00 as libc::c_int == 0 {
            return;
        }
        ds_source = ds_source.offset(1);
        ds_source;
        if !((*pSplat).slope).is_null() {
            if !ds_transmap.is_null() {
                spanfunctype = SPANDRAWFUNC_TILTEDTRANSSOLID as libc::c_int;
            } else {
                spanfunctype = SPANDRAWFUNC_TILTEDSOLID as libc::c_int;
            }
        } else if !ds_transmap.is_null() {
        ...
        if !((*pSplat).slope).is_null() {
            spanfunctype = SPANDRAWFUNC_TILTEDTRANSSPRITE as libc::c_int;
        } else {
            spanfunctype = SPANDRAWFUNC_TRANSSPRITE as libc::c_int;
        }
    } else if !((*pSplat).slope).is_null() {
        spanfunctype = SPANDRAWFUNC_TILTEDSPRITE as libc::c_int;
    } else {
        spanfunctype = SPANDRAWFUNC_SPRITE as libc::c_int;
    }
    if ds_powersoftwo != 0 || ds_solidcolor != 0 {
        spanfunc = spanfuncs[spanfunctype as usize];
    } else {
        spanfunc = spanfuncs_npo2[spanfunctype as usize];
    }
    prepare_rastertab();
    x1 = (*verts.offset(3 as libc::c_int as isize)).x;
    ry1 = (*verts.offset(3 as libc::c_int as isize)).y;
    x2 = (*verts.offset(2 as libc::c_int as isize)).x;
    y2 = (*verts.offset(2 as libc::c_int as isize)).y;
    if y2 > ry1 {
        step = FixedDiv(x2 - x1, y2 - ry1 + 1 as libc::c_int);
    } else if y2 == ry1 {
        step = 0 as libc::c_int;
    } else {
        step = FixedDiv(x2 - x1, ry1 - y2 + 1 as libc::c_int);
    }
    if ry1 < 0 as libc::c_int {
        if step != 0 {
            x1 <<= 16 as libc::c_int;
            x1 += -ry1 * step;
            x1 >>= 16 as libc::c_int;
    ...
    if ry1 >= vid.height {
        if step != 0 {
            x1 <<= 16 as libc::c_int;
            x1 -= (vid.height - 1 as libc::c_int - ry1) * step;
            x1 >>= 16 as libc::c_int;
        }
        ry1 = vid.height - 1 as libc::c_int;
    }
    ...
    if y2 >= vid.height {
        if step != 0 {
            x2 <<= 16 as libc::c_int;
            x2 += (vid.height - 1 as libc::c_int - y2) * step;
            x2 >>= 16 as libc::c_int;
        }
        y2 = vid.height - 1 as libc::c_int;
    }
    ...
    x1 = (*verts.offset(2 as libc::c_int as isize)).x;
    ry1 = (*verts.offset(2 as libc::c_int as isize)).y;
    x2 = (*verts.offset(1 as libc::c_int as isize)).x;
    y2 = (*verts.offset(1 as libc::c_int as isize)).y;
    if y2 > ry1 {
        step = FixedDiv(x2 - x1, y2 - ry1 + 1 as libc::c_int);
    } else if y2 == ry1 {
        step = 0 as libc::c_int;
    ...
    x1 = (*verts.offset(1 as libc::c_int as isize)).x;
    ry1 = (*verts.offset(1 as libc::c_int as isize)).y;
    x2 = (*verts.offset(0 as libc::c_int as isize)).x;
    y2 = (*verts.offset(0 as libc::c_int as isize)).y;
    if y2 > ry1 {
        step = FixedDiv(x2 - x1, y2 - ry1 + 1 as libc::c_int);
    } else if y2 == ry1 {
        step = 0 as libc::c_int;
    ...
    x1 = (*verts.offset(0 as libc::c_int as isize)).x;
    ry1 = (*verts.offset(0 as libc::c_int as isize)).y;
    x2 = (*verts.offset(3 as libc::c_int as isize)).x;
    y2 = (*verts.offset(3 as libc::c_int as isize)).y;
    if y2 > ry1 {
        step = FixedDiv(x2 - x1, y2 - ry1 + 1 as libc::c_int);
    } else if y2 == ry1 {
        step = 0 as libc::c_int;
    ...
    if maxy >= vid.height {
        maxy = vid.height - 1 as libc::c_int;
    }
    y = miny;
    while y <= maxy {
        let mut cliptab: [boolean; 1921] = [0; 1921];
        x1 = rastertab[y as usize].minx >> 16 as libc::c_int;
        x2 = rastertab[y as usize].maxx >> 16 as libc::c_int;
        if x1 > x2 {
            let mut swap: int32_t = x1;
            x1 = x2;
            x2 = swap;
        }
        if !(x1 == -(32767 as libc::c_int) - 1 as libc::c_int
            || x2 == 32767 as libc::c_int)
        {
            if x1 < 0 as libc::c_int {
                x1 = 0 as libc::c_int;
            }
            if x2 >= viewwidth {
                x2 = viewwidth - 1 as libc::c_int;
            }
            if !(x1 >= viewwidth || x2 < 0 as libc::c_int) {
                i = x1;
                while i <= x2 {
                    cliptab[i
                        as usize] = (y >= *mfloorclip.offset(i as isize) as libc::c_int
                        || y <= *mceilingclip.offset(i as isize) as libc::c_int)
                        as libc::c_int;
                    i += 1;
                    i;
                }
                while cliptab[x1 as usize] != 0 {
                    x1 += 1;
                    x1;
                    if x1 >= viewwidth {
                        break;
                ...
                i = x2;
                while i > x1 {
                    if cliptab[i as usize] != 0 {
                        x2 = i - 1 as libc::c_int;
                    }
                    i -= 1;
                    i;
                ...
                if !(x2 < x1) {
                    if ds_solidcolor == 0 && ((*pSplat).slope).is_null() {
                        let mut xstep: fixed_t = 0;
                        let mut ystep: fixed_t = 0;
                        let mut distance: fixed_t = 0;
                        let mut span: fixed_t = 0;
                        let mut angle_0: angle_t = ((*vis).viewpoint.angle)
                            .wrapping_add((*pSplat).angle) >> 19 as libc::c_int;
                        let mut planecos: angle_t = (*finecosine.offset(angle_0 as isize)
                            >> 16 as libc::c_int - 16 as libc::c_int) as angle_t;
                        let mut planesin: angle_t = (finesine[angle_0 as usize]
                            >> 16 as libc::c_int - 16 as libc::c_int) as angle_t;
                        if planeheight != cachedheight[y as usize] {
                            ...
                            cacheddistance[y
                                as usize] = FixedMul(
                                planeheight,
                                *yslope.offset(y as isize),
                            );
                            distance = cacheddistance[y as usize];
                            span = abs(centery - y);
                            if span != 0 {
                                xstep = FixedMul(planesin as fixed_t, planeheight) / span;
                                ystep = FixedMul(planecos as fixed_t, planeheight) / span;
                            } else {
                                ystep = (1 as libc::c_int) << 16 as libc::c_int;
                                xstep = ystep;
                            }
                            cachedxstep[y as usize] = xstep;
                            cachedystep[y as usize] = ystep;
                        } else {
                            distance = cacheddistance[y as usize];
                            xstep = cachedxstep[y as usize];
                            ystep = cachedystep[y as usize];
                        }
                        ds_xstep = FixedDiv(xstep, (*pSplat).xscale);
                        ds_ystep = FixedDiv(ystep, (*pSplat).yscale);
                        ds_xfrac = FixedDiv(
                            offsetx + FixedMul(planecos as fixed_t, distance)
                                + (x1 - centerx) * xstep,
                            (*pSplat).xscale,
                        );
                        ds_yfrac = FixedDiv(
                            offsety - FixedMul(planesin as fixed_t, distance)
                                + (x1 - centerx) * ystep,
                            (*pSplat).yscale,
                        );
                    }
                    ...
                    rastertab[y as usize].minx = 2147483647 as libc::c_int;
                    rastertab[y as usize]
                        .maxx = -(2147483647 as libc::c_int) - 1 as libc::c_int;
                }
            }
        }
        y += 1;
    ...
    if ds_solidcolor == 0 && (*pSplat).angle != 0 && ((*pSplat).slope).is_null() {
        memset(
            cachedheight.as_mut_ptr() as *mut libc::c_void,
            0 as libc::c_int,
            ::core::mem::size_of::<[fixed_t; 1200]>() as libc::c_ulong,
...
unsafe extern "C" fn prepare_rastertab() {
    let mut i: int32_t = 0;
    prastertab = rastertab.as_mut_ptr();
    i = 0 as libc::c_int;
    while i < vid.height {
        rastertab[i as usize].minx = 2147483647 as libc::c_int;
        rastertab[i as usize].maxx = -(2147483647 as libc::c_int) - 1 as libc::c_int;
        i += 1;
        i;
    }
}
