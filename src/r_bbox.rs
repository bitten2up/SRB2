...
pub const RENDERHITBOX_RINGS: C2RustUnnamed_7 = 4;
pub const RENDERHITBOX_INTANGIBLE: C2RustUnnamed_7 = 3;
pub const RENDERHITBOX_ALL: C2RustUnnamed_7 = 2;
pub const RENDERHITBOX_TANGIBLE: C2RustUnnamed_7 = 1;
pub const RENDERHITBOX_OFF: C2RustUnnamed_7 = 0;
...
pub struct vissprite_s {
    ...
    pub viewpoint: C2RustUnnamed_6,
    pub shear: C2RustUnnamed_5,
...
static mut renderhitbox_cons_t: [CV_PossibleValue_t; 6] = [
    {
        let mut init = CV_PossibleValue_s {
            value: RENDERHITBOX_OFF as libc::c_int,
    ...
    {
        let mut init = CV_PossibleValue_s {
            value: RENDERHITBOX_TANGIBLE as libc::c_int,
            strvalue: b"Tangible\0" as *const u8 as *const libc::c_char,
        };
        init
    },
    {
        let mut init = CV_PossibleValue_s {
            value: RENDERHITBOX_ALL as libc::c_int,
    ...
    {
        let mut init = CV_PossibleValue_s {
            value: RENDERHITBOX_INTANGIBLE as libc::c_int,
            strvalue: b"Intangible\0" as *const u8 as *const libc::c_char,
        };
        init
    },
    {
        let mut init = CV_PossibleValue_s {
            value: RENDERHITBOX_RINGS as libc::c_int,
            strvalue: b"Rings\0" as *const u8 as *const libc::c_char,
        };
        init
    },
    {
...
#[no_mangle]
pub static mut cv_renderhitbox: consvar_t = unsafe {
    {
        let mut init = consvar_s {
            name: b"renderhitbox\0" as *const u8 as *const libc::c_char,
            defaultvalue: b"Off\0" as *const u8 as *const libc::c_char,
            flags: CV_CHEAT as libc::c_int | CV_NOTINNET as libc::c_int,
            PossibleValue: renderhitbox_cons_t.as_ptr() as *mut _,
            func: None,
            value: 0 as libc::c_int,
...
#[no_mangle]
pub static mut cv_renderhitboxinterpolation: consvar_t = unsafe {
    {
        let mut init = consvar_s {
            name: b"renderhitbox_interpolation\0" as *const u8 as *const libc::c_char,
...
#[no_mangle]
pub static mut cv_renderhitboxgldepth: consvar_t = unsafe {
    {
        let mut init = consvar_s {
            name: b"renderhitbox_gldepth\0" as *const u8 as *const libc::c_char,
            defaultvalue: b"Off\0" as *const u8 as *const libc::c_char,
            flags: CV_SAVE as libc::c_int,
            PossibleValue: CV_OnOff.as_ptr() as *mut _,
            func: None,
...
#[inline]
unsafe extern "C" fn raster_bbox_seg(
    mut x: int32_t,
    mut y: fixed_t,
    mut h: fixed_t,
    mut pixel: uint8_t,
    ...
    h = y + FixedCeil(abs(h)) / ((1 as libc::c_int) << 16 as libc::c_int);
    if h >= viewheight {
        h = viewheight;
    }
    while y < h {
        *topleft.offset((x + y * vid.width) as isize) = pixel;
        y += 1;
        y;
    }
}
unsafe extern "C" fn draw_bbox_col(
    mut bb: *mut bbox_config,
    mut p: size_t,
    mut tx: fixed_t,
    mut ty: fixed_t,
) {
    let mut col: *mut bbox_col = &mut *((*bb).col).as_mut_ptr().offset(p as isize)
        as *mut bbox_col;
    let mut xscale: fixed_t = 0;
    let mut yscale: fixed_t = 0;
    if ty < (1 as libc::c_int) << 16 as libc::c_int {
        ty = (1 as libc::c_int) << 16 as libc::c_int;
    }
    xscale = FixedDiv(projection, ty);
    yscale = FixedDiv(projectiony, ty);
    (*col)
        .x = (centerxfrac + FixedMul(tx, xscale))
        / ((1 as libc::c_int) << 16 as libc::c_int);
    (*col).y = centeryfrac - FixedMul((*bb).tz, yscale);
    (*col).h = FixedMul((*bb).height, yscale);
    V_DrawFill(
        viewwindowx + (*col).x,
        viewwindowy + (*col).y / ((1 as libc::c_int) << 16 as libc::c_int),
        1 as libc::c_int,
        (*col).h / ((1 as libc::c_int) << 16 as libc::c_int),
        0x40000000 as libc::c_int | (*bb).color as libc::c_int,
    );
}
unsafe extern "C" fn draw_bbox_row(
    mut bb: *mut bbox_config,
    mut p1: size_t,
    mut p2: size_t,
) {
    let mut a: *mut bbox_col = &mut *((*bb).col).as_mut_ptr().offset(p1 as isize)
        as *mut bbox_col;
    let mut b: *mut bbox_col = &mut *((*bb).col).as_mut_ptr().offset(p2 as isize)
        as *mut bbox_col;
    ...
    let mut s1: fixed_t = 0;
    let mut s2: fixed_t = 0;
    if (*a).x > (*b).x {
        let mut c: *mut bbox_col = a;
        a = b;
        b = c;
    }
    x1 = (*a).x;
    x2 = (*b).x;
    if x1 == x2 || x1 >= viewwidth || x2 < 0 as libc::c_int {
        return;
    }
    dx = x2 - x1;
    y1 = (*a).y;
...
#[no_mangle]
pub unsafe extern "C" fn R_GetBoundingBoxColor(mut thing: *mut mobj_t) -> uint8_t {
    let mut flags: uint32_t = (*thing).flags;
    if !((*thing).player).is_null() {
        return 255 as libc::c_int as uint8_t;
    }
    if flags & MF_NOCLIPTHING as libc::c_int as uint32_t != 0 {
        return 7 as libc::c_int as uint8_t;
    }
    if flags & (MF_BOSS as libc::c_int | MF_ENEMY as libc::c_int) as uint32_t != 0 {
        return 35 as libc::c_int as uint8_t;
    }
    if flags & (MF_MISSILE as libc::c_int | MF_PAIN as libc::c_int) as uint32_t != 0 {
        return 54 as libc::c_int as uint8_t;
    }
    if flags & (MF_SPECIAL as libc::c_int | MF_MONITOR as libc::c_int) as uint32_t != 0 {
        return 73 as libc::c_int as uint8_t;
    }
    if flags & MF_PUSHABLE as libc::c_int as uint32_t != 0 {
        return 112 as libc::c_int as uint8_t;
    }
    if flags & MF_SPRING as libc::c_int as uint32_t != 0 {
        return 181 as libc::c_int as uint8_t;
    }
    if flags & MF_NOCLIP as libc::c_int as uint32_t != 0 {
        return 152 as libc::c_int as uint8_t;
    }
    return 0 as libc::c_int as uint8_t;
}
#[no_mangle]
pub unsafe extern "C" fn R_DrawThingBoundingBox(mut vis: *mut vissprite_t) {
    let mut rs: fixed_t = (*vis).scale;
    let mut rc: fixed_t = (*vis).xscale;
    let mut tx: fixed_t = (*vis).gx;
    let mut ty: fixed_t = (*vis).gy;
    let mut bb: bbox_config = {
        let mut init = bbox_config {
            height: (*vis).thingheight,
            tz: (*vis).texturemid,
            col: [bbox_col { x: 0, y: 0, h: 0 }; 4],
            color: R_GetBoundingBoxColor((*vis).mobj),
        };
        init
    };
    draw_bbox_col(&mut bb, 0 as libc::c_int as size_t, tx, ty);
    draw_bbox_col(&mut bb, 1 as libc::c_int as size_t, tx - rc, ty + rs);
    tx += rs;
    ty += rc;
    draw_bbox_col(&mut bb, 2 as libc::c_int as size_t, tx, ty);
    draw_bbox_col(&mut bb, 3 as libc::c_int as size_t, tx - rc, ty + rs);
    draw_bbox_row(&mut bb, 0 as libc::c_int as size_t, 1 as libc::c_int as size_t);
    draw_bbox_row(&mut bb, 1 as libc::c_int as size_t, 3 as libc::c_int as size_t);
    draw_bbox_row(&mut bb, 3 as libc::c_int as size_t, 2 as libc::c_int as size_t);
    draw_bbox_row(&mut bb, 2 as libc::c_int as size_t, 0 as libc::c_int as size_t);
}
unsafe extern "C" fn is_tangible(mut thing: *mut mobj_t) -> boolean {
    if (*thing).flags & MF_NOCLIPTHING as libc::c_int as uint32_t != 0 {
        return false_0 as libc::c_int;
    }
    if (*thing).flags
        & (MF_SPECIAL as libc::c_int | MF_SOLID as libc::c_int
            | MF_SHOOTABLE as libc::c_int | MF_PUSHABLE as libc::c_int
            | MF_BOSS as libc::c_int | MF_MISSILE as libc::c_int
            | MF_SPRING as libc::c_int | MF_BOUNCE as libc::c_int
            | MF_MONITOR as libc::c_int | MF_FIRE as libc::c_int
            | MF_ENEMY as libc::c_int | MF_PAIN as libc::c_int | MF_STICKY as libc::c_int
            | MF_GRENADEBOUNCE as libc::c_int) as uint32_t == 0 as libc::c_uint
    {
        return false_0 as libc::c_int;
    }
...
#[no_mangle]
pub unsafe extern "C" fn R_ThingBoundingBoxVisible(mut thing: *mut mobj_t) -> boolean {
    let mut cvmode: int32_t = cv_renderhitbox.value;
    if multiplayer != 0 {
        return false_0 as libc::c_int;
    }
    's_27: {
    ...
    match cvmode {
        0 => return false_0 as libc::c_int,
        2 => return true_0 as libc::c_int,
        3 => return (is_tangible(thing) == 0) as libc::c_int,
        1 => {
            if (*thing).type_0 as libc::c_uint == MT_RING as libc::c_int as libc::c_uint
            {
                return false_0 as libc::c_int;
            }
        ...
        4 => {
            return ((*thing).type_0 as libc::c_uint
                == MT_RING as libc::c_int as libc::c_uint
                || (*thing).type_0 as libc::c_uint
                    == MT_BLUESPHERE as libc::c_int as libc::c_uint) as libc::c_int;
        }
