...
pub struct lightflash_t {
    pub thinker: thinker_t,
    pub sector: *mut sector_t,
    pub maxlight: int32_t,
    pub minlight: int32_t,
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct strobe_t {
...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct lightlevel_t {
    pub thinker: thinker_t,
    pub sector: *mut sector_t,
    pub sourcelevel: int16_t,
    pub destlevel: int16_t,
    pub fixedcurlevel: fixed_t,
    pub fixedpertic: fixed_t,
    pub timer: int32_t,
}
...
#[no_mangle]
pub unsafe extern "C" fn P_RemoveLighting(mut sector: *mut sector_t) {
    if !((*sector).lightingdata).is_null() {
        P_RemoveThinker(&mut (*((*sector).lightingdata as *mut thinkerdata_t)).thinker);
        (*sector).lightingdata = 0 as *mut libc::c_void;
    }
}
#[no_mangle]
pub unsafe extern "C" fn T_FireFlicker(mut flick: *mut fireflicker_t) {
    let mut amount: int16_t = 0;
    (*flick).count -= 1;
    if (*flick).count != 0 {
        return;
    }
    amount = ((P_RandomByte() as libc::c_int & 3 as libc::c_int) as uint8_t
        as libc::c_int * 16 as libc::c_int) as int16_t;
    if ((*(*flick).sector).lightlevel as libc::c_int - amount as libc::c_int)
        < (*flick).minlight as libc::c_int
    {
        (*(*flick).sector).lightlevel = (*flick).minlight;
    } else {
        (*(*flick).sector)
            .lightlevel = ((*flick).maxlight as libc::c_int - amount as libc::c_int)
            as int16_t;
    }
    (*flick).count = (*flick).resetcount;
}
#[no_mangle]
pub unsafe extern "C" fn P_SpawnAdjustableFireFlicker(
    mut sector: *mut sector_t,
    mut lighta: int16_t,
    mut lightb: int16_t,
    mut length: int32_t,
) -> *mut fireflicker_t {
    let mut flick: *mut fireflicker_t = 0 as *mut fireflicker_t;
    P_RemoveLighting(sector);
    flick = Z_CallocAlign(
        ::core::mem::size_of::<fireflicker_t>() as libc::c_ulong,
        PU_LEVSPEC as libc::c_int,
        0 as *mut libc::c_void,
        ::core::mem::size_of::<*mut libc::c_void>() as libc::c_ulong as int32_t,
    ) as *mut fireflicker_t;
    P_AddThinker(THINK_MAIN, &mut (*flick).thinker);
    (*flick)
        .thinker
        .function
    ...
    (*flick).sector = sector;
    (*flick)
        .maxlight = (if lighta as libc::c_int > lightb as libc::c_int {
        lighta as libc::c_int
    } else {
        lightb as libc::c_int
    }) as int16_t;
    (*flick)
        .minlight = (if (lighta as libc::c_int) < lightb as libc::c_int {
        lighta as libc::c_int
    } else {
        lightb as libc::c_int
    }) as int16_t;
    (*flick).resetcount = length / 4 as libc::c_int;
    (*flick).count = (*flick).resetcount;
    (*sector).lightingdata = flick as *mut libc::c_void;
    if (*flick).resetcount == 0 {
        (*flick).resetcount = 1 as libc::c_int;
    }
    if (*flick).minlight as libc::c_int == (*flick).maxlight as libc::c_int {
        if (*flick).minlight as libc::c_int > 0 as libc::c_int {
            (*flick).minlight -= 1;
            (*flick).minlight;
        }
        if ((*flick).maxlight as libc::c_int) < 255 as libc::c_int {
            (*flick).maxlight += 1;
            (*flick).maxlight;
        }
    }
    (*sector)
        .lightlevel = (if (*flick).minlight as libc::c_int
        > (if ((*flick).maxlight as libc::c_int) < (*sector).lightlevel as libc::c_int {
            (*flick).maxlight as libc::c_int
        } else {
            (*sector).lightlevel as libc::c_int
        })
    {
        (*flick).minlight as libc::c_int
    } else if ((*flick).maxlight as libc::c_int) < (*sector).lightlevel as libc::c_int {
        (*flick).maxlight as libc::c_int
    } else {
        (*sector).lightlevel as libc::c_int
    }) as int16_t;
...
#[no_mangle]
pub unsafe extern "C" fn T_LightningFlash(mut flash: *mut lightflash_t) {
    (*(*flash).sector)
        .lightlevel = ((*(*flash).sector).lightlevel as libc::c_int - 4 as libc::c_int)
        as int16_t;
    if (*(*flash).sector).lightlevel as libc::c_int <= (*flash).minlight {
        (*(*flash).sector).lightlevel = (*flash).minlight as int16_t;
        P_RemoveLighting((*flash).sector);
    }
}
#[no_mangle]
pub unsafe extern "C" fn P_SpawnLightningFlash(mut sector: *mut sector_t) {
    let mut minlight: int32_t = 0;
    let mut flash: *mut lightflash_t = 0 as *mut lightflash_t;
    minlight = (*sector).lightlevel as int32_t;
    if !((*sector).lightingdata).is_null() {
        if (*((*sector).lightingdata as *mut lightflash_t)).thinker.function.acp1
            == ::core::mem::transmute::<
                Option::<unsafe extern "C" fn(*mut lightflash_t) -> ()>,
                actionf_p1,
            >(Some(T_LightningFlash as unsafe extern "C" fn(*mut lightflash_t) -> ()))
        {
            minlight = (*((*sector).lightingdata as *mut lightflash_t)).minlight;
        }
        P_RemoveThinker(&mut (*((*sector).lightingdata as *mut thinkerdata_t)).thinker);
    }
    (*sector).lightingdata = 0 as *mut libc::c_void;
    flash = Z_CallocAlign(
        ::core::mem::size_of::<lightflash_t>() as libc::c_ulong,
        PU_LEVSPEC as libc::c_int,
        0 as *mut libc::c_void,
        ::core::mem::size_of::<*mut libc::c_void>() as libc::c_ulong as int32_t,
    ) as *mut lightflash_t;
    ...
    >(Some(T_LightningFlash as unsafe extern "C" fn(*mut lightflash_t) -> ()));
    (*flash).sector = sector;
    (*flash).maxlight = 255 as libc::c_int;
    (*flash).minlight = minlight;
    (*sector).lightlevel = (*flash).maxlight as int16_t;
    (*sector).lightingdata = flash as *mut libc::c_void;
}
#[no_mangle]
pub unsafe extern "C" fn T_StrobeFlash(mut flash: *mut strobe_t) {
    ...
    if (*(*flash).sector).lightlevel as libc::c_int == (*flash).minlight as libc::c_int {
...
#[no_mangle]
pub unsafe extern "C" fn P_SpawnAdjustableStrobeFlash(
    mut sector: *mut sector_t,
    mut lighta: int16_t,
    mut lightb: int16_t,
    mut darktime: int32_t,
    mut brighttime: int32_t,
    mut inSync: boolean,
) -> *mut strobe_t {
    let mut flash: *mut strobe_t = 0 as *mut strobe_t;
    P_RemoveLighting(sector);
    flash = Z_CallocAlign(
        ::core::mem::size_of::<strobe_t>() as libc::c_ulong,
        PU_LEVSPEC as libc::c_int,
        0 as *mut libc::c_void,
        ::core::mem::size_of::<*mut libc::c_void>() as libc::c_ulong as int32_t,
    ) as *mut strobe_t;
    ...
    if (*flash).minlight as libc::c_int == (*flash).maxlight as libc::c_int {
        (*flash).minlight = 0 as libc::c_int as int16_t;
    }
    if inSync == 0 {
        (*flash)
            .count = (P_RandomByte() as libc::c_int & 7 as libc::c_int)
            + 1 as libc::c_int;
    } else {
        (*flash).count = 1 as libc::c_int;
    }
    (*sector)
        .lightlevel = (if (*flash).minlight as libc::c_int
        > (if ((*flash).maxlight as libc::c_int) < (*sector).lightlevel as libc::c_int {
            (*flash).maxlight as libc::c_int
        } else {
            (*sector).lightlevel as libc::c_int
        })
    {
        (*flash).minlight as libc::c_int
    } else if ((*flash).maxlight as libc::c_int) < (*sector).lightlevel as libc::c_int {
        (*flash).maxlight as libc::c_int
    } else {
        (*sector).lightlevel as libc::c_int
    }) as int16_t;
...
pub unsafe extern "C" fn T_Glow(mut g: *mut glow_t) {
    match (*g).direction as libc::c_int {
        -1 => {
            (*(*g).sector)
                .lightlevel = ((*(*g).sector).lightlevel as libc::c_int
                - (*g).speed as libc::c_int) as int16_t;
            if (*(*g).sector).lightlevel as libc::c_int <= (*g).minlight as libc::c_int {
                (*(*g).sector)
                    .lightlevel = ((*(*g).sector).lightlevel as libc::c_int
                    + (*g).speed as libc::c_int) as int16_t;
                (*g).direction = 1 as libc::c_int as int16_t;
            }
        }
        1 => {
            (*(*g).sector)
                .lightlevel = ((*(*g).sector).lightlevel as libc::c_int
                + (*g).speed as libc::c_int) as int16_t;
            if (*(*g).sector).lightlevel as libc::c_int >= (*g).maxlight as libc::c_int {
                (*(*g).sector)
                    .lightlevel = ((*(*g).sector).lightlevel as libc::c_int
                    - (*g).speed as libc::c_int) as int16_t;
                (*g).direction = -(1 as libc::c_int) as int16_t;
...
#[no_mangle]
pub unsafe extern "C" fn P_SpawnAdjustableGlowingLight(
    mut sector: *mut sector_t,
    mut lighta: int16_t,
    mut lightb: int16_t,
    mut length: int32_t,
) -> *mut glow_t {
    let mut g: *mut glow_t = 0 as *mut glow_t;
    P_RemoveLighting(sector);
    g = Z_CallocAlign(
        ::core::mem::size_of::<glow_t>() as libc::c_ulong,
        PU_LEVSPEC as libc::c_int,
        0 as *mut libc::c_void,
        ::core::mem::size_of::<*mut libc::c_void>() as libc::c_ulong as int32_t,
    ) as *mut glow_t;
    P_AddThinker(THINK_MAIN, &mut (*g).thinker);
    (*g).sector = sector;
    (*g)
        .minlight = (if (lighta as libc::c_int) < lightb as libc::c_int {
        lighta as libc::c_int
    ...
    (*g).direction = 1 as libc::c_int as int16_t;
    (*g).speed = (length / 4 as libc::c_int) as int16_t;
    if (*g).speed as libc::c_int
        > ((*g).maxlight as libc::c_int - (*g).minlight as libc::c_int)
            / 2 as libc::c_int
    {
        (*g)
            .speed = (((*g).maxlight as libc::c_int - (*g).minlight as libc::c_int)
            / 2 as libc::c_int) as int16_t;
    }
    while ((*g).speed as libc::c_int) < 1 as libc::c_int {
        if (*g).minlight as libc::c_int > 0 as libc::c_int {
            (*g).minlight -= 1;
            (*g).minlight;
        }
        if ((*g).maxlight as libc::c_int) < 255 as libc::c_int {
            (*g).maxlight += 1;
            (*g).maxlight;
        }
        (*g)
    ...
    (*sector)
        .lightlevel = (if (*g).minlight as libc::c_int
        > (if ((*g).maxlight as libc::c_int) < (*sector).lightlevel as libc::c_int {
            (*g).maxlight as libc::c_int
        } else {
            (*sector).lightlevel as libc::c_int
        })
    {
        (*g).minlight as libc::c_int
    } else if ((*g).maxlight as libc::c_int) < (*sector).lightlevel as libc::c_int {
        (*g).maxlight as libc::c_int
    } else {
        (*sector).lightlevel as libc::c_int
    }) as int16_t;
    (*sector).lightingdata = g as *mut libc::c_void;
    return g;
}
#[no_mangle]
pub unsafe extern "C" fn P_FadeLightBySector(
    mut sector: *mut sector_t,
    mut destvalue: int32_t,
    mut speed: int32_t,
    mut ticbased: boolean,
) {
    let mut ll: *mut lightlevel_t = 0 as *mut lightlevel_t;
    P_RemoveLighting(sector);
    if ticbased != 0 && speed == 0 || (*sector).lightlevel as libc::c_int == destvalue {
        (*sector).lightlevel = destvalue as int16_t;
        return;
    }
    ll = Z_CallocAlign(
        ::core::mem::size_of::<lightlevel_t>() as libc::c_ulong,
        PU_LEVSPEC as libc::c_int,
        0 as *mut libc::c_void,
        ::core::mem::size_of::<*mut libc::c_void>() as libc::c_ulong as int32_t,
    ) as *mut lightlevel_t;
    (*ll)
        ...
        .function
        .acp1 = ::core::mem::transmute::<
        Option::<unsafe extern "C" fn(*mut lightlevel_t) -> ()>,
        actionf_p1,
    >(Some(T_LightFade as unsafe extern "C" fn(*mut lightlevel_t) -> ()));
    (*sector).lightingdata = ll as *mut libc::c_void;
    P_AddThinker(THINK_MAIN, &mut (*ll).thinker);
    (*ll).sector = sector;
    (*ll).sourcelevel = (*sector).lightlevel;
    (*ll).destlevel = destvalue as int16_t;
    (*ll).fixedcurlevel = ((*sector).lightlevel as libc::c_int) << 16 as libc::c_int;
    if ticbased != 0 {
        (*ll).timer = abs(speed);
        (*ll)
            .fixedpertic = FixedDiv(
            (destvalue << 16 as libc::c_int) - (*ll).fixedcurlevel,
            speed << 16 as libc::c_int,
        );
    } else {
        (*ll)
            .timer = abs(
        ...
        (*ll)
            .fixedpertic = if ((*ll).destlevel as libc::c_int)
            < (*ll).sourcelevel as libc::c_int
        {
            -speed << 16 as libc::c_int
        } else {
            speed << 16 as libc::c_int
...
#[no_mangle]
pub unsafe extern "C" fn P_FadeLight(
    mut tag: int16_t,
    mut destvalue: int32_t,
    mut speed: int32_t,
    mut ticbased: boolean,
    ...
    let mut i: int32_t = 0;
    let mut realdestvalue: int32_t = 0;
    let mut ICNT_367: size_t = 0 as libc::c_int as size_t;
    loop {
        i = Tag_Iterate_Sectors(tag, ICNT_367);
        if !(i >= 0 as libc::c_int) {
            break;
        }
        if force == 0 && ticbased != 0
            && !((*sectors.offset(i as isize)).lightingdata).is_null()
            && (*((*sectors.offset(i as isize)).lightingdata as *mut lightlevel_t))
                .thinker
                .function
                .acp1
                == ::core::mem::transmute::<
                ...
                >(Some(T_LightFade as unsafe extern "C" fn(*mut lightlevel_t) -> ()))
        {
            CONS_Debug(
                0x80 as libc::c_int,
                b"Line type 420 Executor: Fade light thinker already exists, timer: %d\n\0"
                    as *const u8 as *const libc::c_char,
                (*((*sectors.offset(i as isize)).lightingdata as *mut lightlevel_t))
                    .timer,
            );
        } else {
            realdestvalue = if relative != 0 {
                if 0 as libc::c_int
                    > (if (255 as libc::c_int)
                        < (*sectors.offset(i as isize)).lightlevel as libc::c_int
                            + destvalue
                    {
                        255 as libc::c_int
                    } else {
                    ...
                    })
                {
                    ...
                    < (*sectors.offset(i as isize)).lightlevel as libc::c_int + destvalue
                {
                    255 as libc::c_int
                } else {
                    (*sectors.offset(i as isize)).lightlevel as libc::c_int + destvalue
            ...
            P_FadeLightBySector(
                &mut *sectors.offset(i as isize),
                realdestvalue,
                speed,
                ticbased,
            );
        }
        ICNT_367 = ICNT_367.wrapping_add(1);
        ICNT_367;
    };
}
#[no_mangle]
pub unsafe extern "C" fn T_LightFade(mut ll: *mut lightlevel_t) {
    (*ll).timer -= 1;
    if (*ll).timer <= 0 as libc::c_int {
        (*(*ll).sector).lightlevel = (*ll).destlevel;
        P_RemoveLighting((*ll).sector);
        return;
    }
    (*ll).fixedcurlevel = (*ll).fixedcurlevel + (*ll).fixedpertic;
    (*(*ll).sector).lightlevel = ((*ll).fixedcurlevel >> 16 as libc::c_int) as int16_t;
}
