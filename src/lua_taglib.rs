...
extern "C" {
    ...
    fn Tag_Share(list1: *const taglist_t, list2: *const taglist_t) -> boolean;
    fn Tag_Compare(list1: *const taglist_t, list2: *const taglist_t) -> boolean;
    fn Tag_SectorFSet(id: size_t, tag: mtag_t);
    static mut tags_available: [bitarray_t; 0];
    static mut num_tags: size_t;
    static mut tags_sectors: [*mut taggroup_t; 0];
    fn Taggroup_Add(garray: *mut *mut taggroup_t, tag: mtag_t, id: size_t);
    fn Taggroup_Remove(garray: *mut *mut taggroup_t, tag: mtag_t, id: size_t);
    fn Taggroup_Count(group: *const taggroup_t) -> size_t;
    fn Taggroup_Iterate(
...
#[derive(Copy, Clone)]
#[repr(C)]
pub struct element_iterator_state {
    pub tag: mtag_t,
    pub p: size_t,
}
pub const up_garray: C2RustUnnamed_1 = -10003;
pub const up_max_elements: C2RustUnnamed_1 = -10004;
pub const up_sizeof_element: C2RustUnnamed_1 = -10006;
pub const up_element_array: C2RustUnnamed_1 = -10005;
pub const up_meta: C2RustUnnamed_1 = -10007;
...
unsafe extern "C" fn tag_iterator(mut L: *mut lua_State) -> libc::c_int {
    let mut tag: int32_t = if lua_type(L, 2 as libc::c_int) == 0 as libc::c_int {
        -(1 as libc::c_int)
    } else {
        lua_tonumber(L, 2 as libc::c_int)
    };
    loop {
        tag += 1;
        if tag >= 65535 as libc::c_int {
            return 0 as libc::c_int;
        }
        if !(in_bit_array(tags_available.as_mut_ptr(), tag) == 0) {
            break;
        }
    }
...
unsafe extern "C" fn next_element(
    mut L: *mut lua_State,
    tag: mtag_t,
    p: size_t,
) -> int32_t {
    let mut garray: *mut *mut taggroup_t = lua_touserdata(L, up_garray as libc::c_int)
        as *mut *mut taggroup_t;
    let mut max_elements: *const size_t = lua_touserdata(
        L,
        up_max_elements as libc::c_int,
    ) as *const size_t;
    return Taggroup_Iterate(garray, *max_elements, tag, p);
}
unsafe extern "C" fn push_element(
    mut L: *mut lua_State,
    mut element: *mut libc::c_void,
) {
    if LUA_RawPushUserdata(L, element) as libc::c_uint
        == LPUSHED_NEW as libc::c_int as libc::c_uint
    {
        lua_pushvalue(L, up_meta as libc::c_int);
        lua_setmetatable(L, -(2 as libc::c_int));
    }
}
unsafe extern "C" fn push_next_element(mut L: *mut lua_State, element: int32_t) {
    let mut element_array: *mut libc::c_char = *(lua_touserdata(
        L,
        up_element_array as libc::c_int,
    ) as *mut *mut libc::c_char);
    let sizeof_element: size_t = lua_tonumber(L, up_sizeof_element as libc::c_int)
        as size_t;
    push_element(
        L,
        &mut *element_array.offset((element as size_t * sizeof_element) as isize)
            as *mut libc::c_char as *mut libc::c_void,
    );
}
unsafe extern "C" fn element_iterator(mut L: *mut lua_State) -> libc::c_int {
    let mut state: *mut element_iterator_state = lua_touserdata(L, 1 as libc::c_int)
        as *mut element_iterator_state;
    if lua_type(L, 3 as libc::c_int) <= 0 as libc::c_int {
        (*state).p = 0 as libc::c_int as size_t;
    }
    (*state).p = ((*state).p).wrapping_add(1);
    lua_pushnumber(L, (*state).p as lua_Number);
    lua_gettable(L, 1 as libc::c_int);
    return 1 as libc::c_int;
}
unsafe extern "C" fn lib_iterateTags(mut L: *mut lua_State) -> libc::c_int {
    if lua_gettop(L) < 2 as libc::c_int {
        lua_pushcclosure(
            L,
            Some(tag_iterator as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
            0 as libc::c_int,
        );
        return 1 as libc::c_int;
    } else {
...
unsafe extern "C" fn lib_numTags(mut L: *mut lua_State) -> libc::c_int {
    lua_pushnumber(L, num_tags as lua_Number);
    return 1 as libc::c_int;
}
unsafe extern "C" fn lib_getTaggroup(mut L: *mut lua_State) -> libc::c_int {
    let mut state: *mut element_iterator_state = 0 as *mut element_iterator_state;
    let mut tag: mtag_t = 0;
    if lua_gettop(L) > 1 as libc::c_int {
        return luaL_error(
            L,
    ...
    if lua_type(L, 1 as libc::c_int) <= 0 as libc::c_int {
        tag = -(1 as libc::c_int) as mtag_t;
    } else {
        tag = lua_tonumber(L, 1 as libc::c_int) as mtag_t;
        (tag as libc::c_int >= -(1 as libc::c_int)
            || luaL_argerror(
                L,
                1 as libc::c_int,
                b"tag out of range\0" as *const u8 as *const libc::c_char,
            ) != 0) as libc::c_int;
    }
    state = lua_newuserdata(
        L,
        ::core::mem::size_of::<element_iterator_state>() as libc::c_ulong,
    ) as *mut element_iterator_state;
    (*state).tag = tag;
    (*state).p = 0 as libc::c_int as size_t;
...
unsafe extern "C" fn lib_getTaggroupElement(mut L: *mut lua_State) -> libc::c_int {
    let p: size_t = (luaL_checknumber(L, 2 as libc::c_int) - 1 as libc::c_int) as size_t;
    let tag: mtag_t = *(lua_touserdata(L, 1 as libc::c_int) as *mut mtag_t);
    let element: int32_t = next_element(L, tag, p);
    if element == -(1 as libc::c_int) {
        return 0 as libc::c_int
    } else {
...
unsafe extern "C" fn lib_numTaggroupElements(mut L: *mut lua_State) -> libc::c_int {
    let tag: mtag_t = *(lua_touserdata(L, 1 as libc::c_int) as *mut mtag_t);
    if tag as libc::c_int == -(1 as libc::c_int) {
        lua_pushnumber(
            L,
            *(lua_touserdata(L, up_max_elements as libc::c_int) as *mut size_t)
                as lua_Number,
        );
    } else {
        let mut garray: *mut *const taggroup_t = lua_touserdata(
            L,
            up_garray as libc::c_int,
        ) as *mut *const taggroup_t;
        lua_pushnumber(L, Taggroup_Count(*garray.offset(tag as isize)) as lua_Number);
    }
    return 1 as libc::c_int;
}
static mut meta_ref: [libc::c_int; 2] = [0; 2];
unsafe extern "C" fn has_valid_field(mut L: *mut lua_State) -> libc::c_int {
    let mut equal: libc::c_int = 0;
    lua_rawgeti(L, -(10001 as libc::c_int), 1 as libc::c_int);
    equal = lua_rawequal(L, 2 as libc::c_int, -(1 as libc::c_int));
    lua_settop(L, -(1 as libc::c_int) - 1 as libc::c_int);
    return equal;
}
unsafe extern "C" fn valid_taglist(
    mut L: *mut lua_State,
    mut idx: libc::c_int,
    mut getting: boolean,
) -> *mut taglist_t {
    let mut list: *mut taglist_t = *(lua_touserdata(L, idx) as *mut *mut taglist_t);
    if list.is_null() {
        if getting != 0 && has_valid_field(L) != 0 {
            lua_pushboolean(L, 0 as libc::c_int);
        } else {
            luaL_error(
                L,
                b"accessed taglist doesn't exist anymore, please check 'valid' before using taglist.\0"
                    as *const u8 as *const libc::c_char,
            );
        }
        return 0 as *mut taglist_t;
    } else {
        return list
    };
}
unsafe extern "C" fn check_taglist(
    mut L: *mut lua_State,
    mut idx: libc::c_int,
) -> *mut taglist_t {
    if lua_isuserdata(L, idx) != 0 && lua_getmetatable(L, idx) != 0 {
        lua_rawgeti(L, -(10000 as libc::c_int), meta_ref[0 as libc::c_int as usize]);
        lua_rawgeti(L, -(10000 as libc::c_int), meta_ref[1 as libc::c_int as usize]);
        if lua_rawequal(L, -(3 as libc::c_int), -(2 as libc::c_int)) != 0
            || lua_rawequal(L, -(3 as libc::c_int), -(1 as libc::c_int)) != 0
        {
            lua_settop(L, -(3 as libc::c_int) - 1 as libc::c_int);
            return valid_taglist(L, idx, false_0 as libc::c_int);
        }
    }
    luaL_argerror(L, idx, b"must be a tag list\0" as *const u8 as *const libc::c_char);
    return 0 as *mut libc::c_void as *mut taglist_t;
}
unsafe extern "C" fn taglist_get(mut L: *mut lua_State) -> libc::c_int {
    let mut list: *const taglist_t = valid_taglist(
        L,
        1 as libc::c_int,
    ...
    if lua_isnumber(L, 2 as libc::c_int) != 0 {
        let i: size_t = lua_tonumber(L, 2 as libc::c_int) as size_t;
        if !list.is_null() && i <= (*list).count as size_t {
            lua_pushnumber(
                L,
                *((*list).tags)
...
unsafe extern "C" fn taglist_len(mut L: *mut lua_State) -> libc::c_int {
    let mut list: *const taglist_t = valid_taglist(
        L,
        1 as libc::c_int,
        false_0 as libc::c_int,
    );
    lua_pushnumber(L, (*list).count as lua_Number);
    return 1 as libc::c_int;
}
unsafe extern "C" fn taglist_equal(mut L: *mut lua_State) -> libc::c_int {
    let mut lhs: *const taglist_t = check_taglist(L, 1 as libc::c_int);
    let mut rhs: *const taglist_t = check_taglist(L, 2 as libc::c_int);
    lua_pushboolean(L, Tag_Compare(lhs, rhs));
    return 1 as libc::c_int;
}
unsafe extern "C" fn taglist_iterator(mut L: *mut lua_State) -> libc::c_int {
    let mut list: *const taglist_t = valid_taglist(
        L,
        1 as libc::c_int,
        false_0 as libc::c_int,
    );
    let i: size_t = (1 as libc::c_int
        + lua_tonumber(L, -(10002 as libc::c_int) - 1 as libc::c_int)) as size_t;
    if i <= (*list).count as size_t {
        lua_pushnumber(
            L,
            *((*list).tags).offset(i.wrapping_sub(1 as libc::c_int as size_t) as isize)
                as lua_Number,
        );
        lua_pushnumber(L, i as lua_Number);
        lua_replace(L, -(10002 as libc::c_int) - 1 as libc::c_int);
        return 1 as libc::c_int;
    } else {
        return 0 as libc::c_int
    };
}
unsafe extern "C" fn taglist_iterate(mut L: *mut lua_State) -> libc::c_int {
    check_taglist(L, 1 as libc::c_int);
    lua_pushnumber(L, 0 as libc::c_int);
    lua_pushcclosure(
        L,
        Some(taglist_iterator as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
        1 as libc::c_int,
    );
    lua_pushvalue(L, 1 as libc::c_int);
    return 2 as libc::c_int;
}
unsafe extern "C" fn taglist_find(mut L: *mut lua_State) -> libc::c_int {
    let mut list: *const taglist_t = check_taglist(L, 1 as libc::c_int);
    let tag: mtag_t = luaL_checknumber(L, 2 as libc::c_int) as mtag_t;
    lua_pushboolean(L, Tag_Find(list, tag));
    return 1 as libc::c_int;
}
unsafe extern "C" fn taglist_shares(mut L: *mut lua_State) -> libc::c_int {
    let mut lhs: *const taglist_t = check_taglist(L, 1 as libc::c_int);
    let mut rhs: *const taglist_t = check_taglist(L, 2 as libc::c_int);
    lua_pushboolean(L, Tag_Share(lhs, rhs));
    return 1 as libc::c_int;
}
unsafe extern "C" fn sector_of_taglist(mut list: *mut taglist_t) -> size_t {
    return ((list as *mut libc::c_char).offset(-(24 as libc::c_ulong as isize))
        as *mut sector_t)
        .offset_from(sectors) as libc::c_long as size_t;
}
unsafe extern "C" fn this_taglist(mut L: *mut lua_State) -> libc::c_int {
    lua_settop(L, 1 as libc::c_int);
    return 1 as libc::c_int;
}
unsafe extern "C" fn taglist_add(mut L: *mut lua_State) -> libc::c_int {
    let mut list: *mut taglist_t = *(luaL_checkudata(
        L,
        1 as libc::c_int,
        b"sector_t.taglist\0" as *const u8 as *const libc::c_char,
    ) as *mut *mut taglist_t);
    let tag: mtag_t = luaL_checknumber(L, 2 as libc::c_int) as mtag_t;
    if Tag_Find(list, tag) == 0 {
        Taggroup_Add(tags_sectors.as_mut_ptr(), tag, sector_of_taglist(list));
        Tag_Add(list, tag);
    }
    return this_taglist(L);
}
unsafe extern "C" fn taglist_remove(mut L: *mut lua_State) -> libc::c_int {
    let mut list: *mut taglist_t = *(luaL_checkudata(
        L,
        1 as libc::c_int,
        b"sector_t.taglist\0" as *const u8 as *const libc::c_char,
    ...
    while i < (*list).count as size_t {
        if *((*list).tags).offset(i as isize) as libc::c_int == tag as libc::c_int {
            if (*list).count as libc::c_int > 1 as libc::c_int {
                memmove(
                    &mut *((*list).tags).offset(i as isize) as *mut mtag_t
                        as *mut libc::c_void,
                    &mut *((*list).tags)
                        .offset(i.wrapping_add(1 as libc::c_int as size_t) as isize)
                        as *mut mtag_t as *const libc::c_void,
                    (((*list).count as libc::c_int - 1 as libc::c_int) as size_t)
                        .wrapping_sub(i)
                ...
                (*list)
                    .tags = Z_ReallocAlign(
                    (*list).tags as *mut libc::c_void,
                    ((*list).count as libc::c_ulong)
                        .wrapping_mul(::core::mem::size_of::<mtag_t>() as libc::c_ulong),
                ...
                Taggroup_Remove(tags_sectors.as_mut_ptr(), tag, sector_of_taglist(list));
            } else {
                Tag_SectorFSet(sector_of_taglist(list), 0 as libc::c_int as mtag_t);
            }
            break;
        } else {
            i = i.wrapping_add(1);
...
#[no_mangle]
pub unsafe extern "C" fn LUA_InsertTaggroupIterator(
    mut L: *mut lua_State,
    ...
    lua_createtable(L, 0 as libc::c_int, 3 as libc::c_int);
    lua_pushlightuserdata(L, garray as *mut libc::c_void);
    lua_pushlightuserdata(L, max_elements as *mut libc::c_void);
    lua_pushvalue(L, -(2 as libc::c_int));
    lua_pushvalue(L, -(2 as libc::c_int));
    lua_pushlightuserdata(L, element_array);
    lua_pushnumber(L, sizeof_element as lua_Number);
    lua_getfield(L, -(10000 as libc::c_int), meta);
    lua_pushcclosure(
        L,
        Some(
            lib_getTaggroupElement as unsafe extern "C" fn(*mut lua_State) -> libc::c_int,
        ),
        5 as libc::c_int,
    );
    lua_setfield(
    ...
    lua_pushcclosure(
        L,
        Some(element_iterator as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
        0 as libc::c_int,
    );
    lua_setfield(
        L,
    ...
    lua_pushcclosure(
        L,
        Some(lib_getTaggroup as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
        1 as libc::c_int,
    );
    lua_setfield(
        L,
        -(2 as libc::c_int),
        b"tagged\0" as *const u8 as *const libc::c_char,
    );
}
static mut taglist_lib: [luaL_Reg; 6] = unsafe {
    [
        {
            let mut init = luaL_Reg {
                name: b"iterate\0" as *const u8 as *const libc::c_char,
                func: Some(
                    taglist_iterate
                        as unsafe extern "C" fn(*mut lua_State) -> libc::c_int,
                ),
        ...
        },
        {
            let mut init = luaL_Reg {
                ...
                func: Some(
                    taglist_find as unsafe extern "C" fn(*mut lua_State) -> libc::c_int,
                ),
            };
            init
        },
        {
            let mut init = luaL_Reg {
                name: b"shares\0" as *const u8 as *const libc::c_char,
                func: Some(
                    taglist_shares as unsafe extern "C" fn(*mut lua_State) -> libc::c_int,
                ),
            };
            init
        },
        {
            let mut init = luaL_Reg {
                ...
                func: Some(
                    taglist_add as unsafe extern "C" fn(*mut lua_State) -> libc::c_int,
                ),
            };
            init
        },
        {
            let mut init = luaL_Reg {
                ...
                func: Some(
                    taglist_remove as unsafe extern "C" fn(*mut lua_State) -> libc::c_int,
                ),
            };
            init
        },
...
unsafe extern "C" fn open_taglist(mut L: *mut lua_State) {
    luaL_register(
        L,
        b"taglist\0" as *const u8 as *const libc::c_char,
        taglist_lib.as_mut_ptr(),
    );
    lua_getfield(L, -(1 as libc::c_int), b"find\0" as *const u8 as *const libc::c_char);
    lua_setfield(L, -(2 as libc::c_int), b"has\0" as *const u8 as *const libc::c_char);
}
unsafe extern "C" fn set_taglist_metatable(
    mut L: *mut lua_State,
    mut meta: *const libc::c_char,
) -> libc::c_int {
    ...
    lua_pushcclosure(
        L,
        Some(taglist_get as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
        0 as libc::c_int,
    );
    ...
    lua_pushcclosure(
        L,
        Some(taglist_len as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
        0 as libc::c_int,
    );
    lua_setfield(L, -(2 as libc::c_int), b"__len\0" as *const u8 as *const libc::c_char);
    lua_pushcclosure(
        L,
        Some(taglist_equal as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
        0 as libc::c_int,
    );
    lua_setfield(L, -(2 as libc::c_int), b"__eq\0" as *const u8 as *const libc::c_char);
    return luaL_ref(L, -(10000 as libc::c_int));
}
#[no_mangle]
pub unsafe extern "C" fn LUA_TagLib(mut L: *mut lua_State) -> libc::c_int {
    lua_newuserdata(L, 0 as libc::c_int as size_t);
    lua_createtable(L, 0 as libc::c_int, 2 as libc::c_int);
    lua_createtable(L, 0 as libc::c_int, 1 as libc::c_int);
    lua_pushcclosure(
        L,
        Some(lib_iterateTags as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
        0 as libc::c_int,
    );
    lua_setfield(
        L,
    ...
    lua_pushcclosure(
        L,
        Some(lib_numTags as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
        0 as libc::c_int,
    );
    lua_setfield(L, -(2 as libc::c_int), b"__len\0" as *const u8 as *const libc::c_char);
    lua_setmetatable(L, -(2 as libc::c_int));
    ...
    open_taglist(L);
    meta_ref[0 as libc::c_int
        as usize] = set_taglist_metatable(
        L,
        b"TAGLIST\0" as *const u8 as *const libc::c_char,
    );
    meta_ref[1 as libc::c_int
        as usize] = set_taglist_metatable(
        L,
        b"sector_t.taglist\0" as *const u8 as *const libc::c_char,
    );
