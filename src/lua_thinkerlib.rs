...
pub const NUM_THINKERLISTS: C2RustUnnamed = 5;
pub const THINK_PRECIP: C2RustUnnamed = 4;
pub const THINK_DYNSLOPE: C2RustUnnamed = 3;
pub const THINK_MOBJ: C2RustUnnamed = 2;
pub const THINK_MAIN: C2RustUnnamed = 1;
pub const THINK_POLYOBJ: C2RustUnnamed = 0;
pub type lua_CFunction = Option::<unsafe extern "C" fn(*mut lua_State) -> libc::c_int>;
#[derive(Copy, Clone)]
#[repr(C)]
pub struct iterationState {
    pub filter: actionf_p1,
    pub next: libc::c_int,
}
unsafe extern "C" fn iterationState_gc(mut L: *mut lua_State) -> libc::c_int {
    let mut it: *mut iterationState = luaL_checkudata(
        L,
        -(1 as libc::c_int),
        b"iteration state\0" as *const u8 as *const libc::c_char,
    ) as *mut iterationState;
    if (*it).next != -(1 as libc::c_int) {
        luaL_unref(L, -(10000 as libc::c_int), (*it).next);
        (*it).next = -(1 as libc::c_int);
    }
    return 0 as libc::c_int;
}
unsafe extern "C" fn lib_iterateThinkers(mut L: *mut lua_State) -> libc::c_int {
    let mut th: *mut thinker_t = 0 as *mut thinker_t;
    let mut next: *mut thinker_t = 0 as *mut thinker_t;
    let mut it: *mut iterationState = 0 as *mut iterationState;
    ...
    if lua_type(L, 2 as libc::c_int) == 0 as libc::c_int {
        th = &mut *thlist.as_mut_ptr().offset(THINK_MOBJ as libc::c_int as isize)
            as *mut thinker_t;
    } else if lua_isuserdata(L, 2 as libc::c_int) != 0 {
        if lua_type(L, 2 as libc::c_int) == 2 as libc::c_int {
            th = lua_touserdata(L, 2 as libc::c_int) as *mut thinker_t;
        } else {
            th = *(lua_touserdata(L, -(1 as libc::c_int)) as *mut *mut thinker_t);
            if th.is_null() {
                if (*it).next == -(1 as libc::c_int) {
                    return 0 as libc::c_int;
                }
                lua_rawgeti(L, -(10000 as libc::c_int), (*it).next);
                if lua_type(L, -(1 as libc::c_int)) == 2 as libc::c_int {
                    next = lua_touserdata(L, -(1 as libc::c_int)) as *mut thinker_t;
                } else {
                    next = *(lua_touserdata(L, -(1 as libc::c_int))
    ...
    luaL_unref(L, -(10000 as libc::c_int), (*it).next);
    (*it).next = -(1 as libc::c_int);
    if !th.is_null() && next.is_null() {
        next = (*th).next;
    }
    if next.is_null() {
        return luaL_error(
            L,
            b"next thinker invalidated during iteration\0" as *const u8
    ...
    while next
        != &mut *thlist.as_mut_ptr().offset(THINK_MOBJ as libc::c_int as isize)
            as *mut thinker_t
    {
        if ((*it).filter).is_none() || (*next).function.acp1 == (*it).filter {
            if (*next).function.acp1
                == ::core::mem::transmute::<
                ...
                >(Some(P_MobjThinker as unsafe extern "C" fn(*mut mobj_t) -> ()))
            {
                ...
                lua_pushlightuserdata(L, next as *mut libc::c_void);
            }
            if (*next).next
                != &mut *thlist.as_mut_ptr().offset(THINK_MOBJ as libc::c_int as isize)
                    as *mut thinker_t
            {
                if (*(*next).next).function.acp1
                    == ::core::mem::transmute::<
                        Option::<unsafe extern "C" fn(*mut mobj_t) -> ()>,
                        actionf_p1,
                    >(Some(P_MobjThinker as unsafe extern "C" fn(*mut mobj_t) -> ()))
                {
                    LUA_PushUserdata(
                        L,
                        (*next).next as *mut libc::c_void,
                        b"MOBJ_T*\0" as *const u8 as *const libc::c_char,
                    );
                } else {
                    lua_pushlightuserdata(L, (*next).next as *mut libc::c_void);
                }
                (*it).next = luaL_ref(L, -(10000 as libc::c_int));
            }
...
unsafe extern "C" fn lib_startIterate(mut L: *mut lua_State) -> libc::c_int {
    let mut it: *mut iterationState = 0 as *mut iterationState;
    if !(gamestate as libc::c_uint == GS_LEVEL as libc::c_int as libc::c_uint
        || titlemapinaction as libc::c_int != 0)
    {
    ...
    it = lua_newuserdata(L, ::core::mem::size_of::<iterationState>() as libc::c_ulong)
        as *mut iterationState;
    lua_getfield(
        L,
        -(10000 as libc::c_int),
...
#[no_mangle]
pub unsafe extern "C" fn LUA_ThinkerLib(mut L: *mut lua_State) -> libc::c_int {
    luaL_newmetatable(L, b"iteration state\0" as *const u8 as *const libc::c_char);
    lua_pushcclosure(
        L,
        Some(iterationState_gc as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
        0 as libc::c_int,
    );
    ...
    lua_pushcclosure(
        L,
        Some(lib_iterateThinkers as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
        0 as libc::c_int,
    );
    lua_pushcclosure(
        L,
        Some(lib_startIterate as unsafe extern "C" fn(*mut lua_State) -> libc::c_int),
        1 as libc::c_int,
    );
    lua_setfield(
        L,
    ...
    lua_setfield(
        L,
        -(10002 as libc::c_int),
        b"mobjs\0" as *const u8 as *const libc::c_char,
    );
    return 0 as libc::c_int;
}
