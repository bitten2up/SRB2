...
#[no_mangle]
pub static mut rollcosang: [fixed_t; 72] = [0; 72];
#[no_mangle]
pub static mut rollsinang: [fixed_t; 72] = [0; 72];
#[no_mangle]
pub unsafe extern "C" fn R_ModelRotationAngle(
    mut interp: *mut interpmobjstate_t,
) -> angle_t {
    return (*interp).spriteroll;
}
#[no_mangle]
pub unsafe extern "C" fn R_SpriteRotationAngle(
    mut interp: *mut interpmobjstate_t,
) -> angle_t {
    return R_ModelRotationAngle(interp);
}
#[no_mangle]
pub unsafe extern "C" fn R_GetRollAngle(mut rollangle: angle_t) -> int32_t {
    let mut ra: int32_t = AngleFixed(rollangle) >> 16 as libc::c_int;
    ra += 360 as libc::c_int / 72 as libc::c_int / 2 as libc::c_int;
    ra /= 360 as libc::c_int / 72 as libc::c_int;
    ra %= 72 as libc::c_int;
    return ra;
}
#[no_mangle]
pub unsafe extern "C" fn Patch_GetRotated(
    mut patch: *mut patch_t,
    mut angle: int32_t,
    mut flip: boolean,
) -> *mut patch_t {
    let mut rotsprite: *mut rotsprite_t = (*patch).rotated;
    if rotsprite.is_null() || angle < 1 as libc::c_int || angle >= 72 as libc::c_int {
        return 0 as *mut patch_t;
    }
    if flip != 0 {
        angle += (*rotsprite).angles;
    }
    return *((*rotsprite).patches).offset(angle as isize) as *mut patch_t;
}
#[no_mangle]
pub unsafe extern "C" fn Patch_GetRotatedSprite(
    mut sprite: *mut spriteframe_t,
    mut frame: size_t,
    mut spriteangle: size_t,
    mut flip: boolean,
    mut adjustfeet: boolean,
    mut info: *mut libc::c_void,
    mut rotationangle: int32_t,
) -> *mut patch_t {
    let mut rotsprite: *mut rotsprite_t = 0 as *mut rotsprite_t;
    let mut sprinfo: *mut spriteinfo_t = info as *mut spriteinfo_t;
    let mut idx: int32_t = rotationangle;
    let mut type_0: uint8_t = (if adjustfeet != 0 {
        1 as libc::c_int
    } else {
        0 as libc::c_int
    }) as uint8_t;
    if rotationangle < 1 as libc::c_int || rotationangle >= 72 as libc::c_int {
        return 0 as *mut patch_t;
    }
    rotsprite = (*sprite).rotated[type_0 as usize][spriteangle as usize];
    if rotsprite.is_null() {
        rotsprite = RotatedPatch_Create(72 as libc::c_int);
        (*sprite).rotated[type_0 as usize][spriteangle as usize] = rotsprite;
    }
    if flip != 0 {
        idx += (*rotsprite).angles;
    }
    if (*((*rotsprite).patches).offset(idx as isize)).is_null() {
        let mut patch: *mut patch_t = 0 as *mut patch_t;
        let mut xpivot: int32_t = 0 as libc::c_int;
        let mut ypivot: int32_t = 0 as libc::c_int;
        let mut lump: lumpnum_t = (*sprite).lumppat[spriteangle as usize];
        if lump == 4294967295 as libc::c_uint {
            return 0 as *mut patch_t;
        }
        patch = W_CachePatchNum(lump, PU_SPRITE as libc::c_int) as *mut patch_t;
        if (*sprinfo).available != 0 {
            xpivot = (*sprinfo).pivot[frame as usize].x;
            ypivot = (*sprinfo).pivot[frame as usize].y;
        } else {
            xpivot = (*patch).leftoffset as int32_t;
            ypivot = (*patch).height as libc::c_int / 2 as libc::c_int;
        }
        RotatedPatch_DoRotation(rotsprite, patch, rotationangle, xpivot, ypivot, flip);
        if adjustfeet != 0 {
            let ref mut fresh0 = (*(*((*rotsprite).patches).offset(idx as isize)
                as *mut patch_t))
                .topoffset;
            *fresh0 = (*fresh0 as libc::c_int
                + ((4 as libc::c_int) << 16 as libc::c_int >> 16 as libc::c_int))
                as int16_t;
        }
    }
    return *((*rotsprite).patches).offset(idx as isize) as *mut patch_t;
}
#[no_mangle]
pub unsafe extern "C" fn Patch_Rotate(
    mut patch: *mut patch_t,
    mut angle: int32_t,
    mut xpivot: int32_t,
    mut ypivot: int32_t,
    ...
    if ((*patch).rotated).is_null() {
        (*patch).rotated = RotatedPatch_Create(72 as libc::c_int);
    }
    RotatedPatch_DoRotation((*patch).rotated, patch, angle, xpivot, ypivot, flip);
}
#[no_mangle]
pub unsafe extern "C" fn RotatedPatch_Create(
    mut numangles: int32_t,
) -> *mut rotsprite_t {
    let mut rotsprite: *mut rotsprite_t = Z_CallocAlign(
        ::core::mem::size_of::<rotsprite_t>() as libc::c_ulong,
    ...
    (*rotsprite)
        .patches = Z_CallocAlign(
        (((*rotsprite).angles * 2 as libc::c_int) as libc::c_ulong)
...
unsafe extern "C" fn RotatedPatch_CalculateDimensions(
    mut width: int32_t,
    mut height: int32_t,
    mut ca: fixed_t,
    mut sa: fixed_t,
    mut newwidth: *mut int32_t,
    mut newheight: *mut int32_t,
) {
    let mut fixedwidth: fixed_t = width * ((1 as libc::c_int) << 16 as libc::c_int);
    let mut fixedheight: fixed_t = height * ((1 as libc::c_int) << 16 as libc::c_int);
    let mut w1: fixed_t = abs(FixedMul(fixedwidth, ca) - FixedMul(fixedheight, sa));
    let mut w2: fixed_t = abs(FixedMul(-fixedwidth, ca) - FixedMul(fixedheight, sa));
    let mut h1: fixed_t = abs(FixedMul(fixedwidth, sa) + FixedMul(fixedheight, ca));
    let mut h2: fixed_t = abs(FixedMul(-fixedwidth, sa) + FixedMul(fixedheight, ca));
    w1 = FixedInt(
        FixedCeil(w1 + ((1 as libc::c_int) << 16 as libc::c_int) / 2 as libc::c_int),
    );
    w2 = FixedInt(
        FixedCeil(w2 + ((1 as libc::c_int) << 16 as libc::c_int) / 2 as libc::c_int),
    );
    h1 = FixedInt(
        FixedCeil(h1 + ((1 as libc::c_int) << 16 as libc::c_int) / 2 as libc::c_int),
    );
    h2 = FixedInt(
        FixedCeil(h2 + ((1 as libc::c_int) << 16 as libc::c_int) / 2 as libc::c_int),
    );
    *newwidth = if width > (if w1 > w2 { w1 } else { w2 }) {
        width
    } else if w1 > w2 {
        w1
    } else {
    ...
    *newheight = if height > (if h1 > h2 { h1 } else { h2 }) {
        height
    } else if h1 > h2 {
        h1
    } else {
...
#[no_mangle]
pub unsafe extern "C" fn RotatedPatch_DoRotation(
    mut rotsprite: *mut rotsprite_t,
    mut patch: *mut patch_t,
    mut angle: int32_t,
    mut xpivot: int32_t,
    mut ypivot: int32_t,
    ...
    let mut rotated: *mut patch_t = 0 as *mut patch_t;
    let mut rawdst: *mut uint16_t = 0 as *mut uint16_t;
    let mut rawconv: *mut uint16_t = 0 as *mut uint16_t;
    let mut size: size_t = 0;
    let mut bflip: pictureflags_t = (if flip != 0 {
        PICFLAGS_XFLIP as libc::c_int
    } else {
        0 as libc::c_int
    }) as pictureflags_t;
    let mut width: int32_t = (*patch).width as int32_t;
    let mut height: int32_t = (*patch).height as int32_t;
    let mut leftoffset: int32_t = (*patch).leftoffset as int32_t;
    let mut newwidth: int32_t = 0;
    let mut newheight: int32_t = 0;
    let mut ca: fixed_t = rollcosang[angle as usize];
    let mut sa: fixed_t = rollsinang[angle as usize];
    let mut xcenter: fixed_t = 0;
    let mut ycenter: fixed_t = 0;
    let mut idx: int32_t = angle;
    let mut x: int32_t = 0;
    let mut y: int32_t = 0;
    ...
    let mut oy: int32_t = 0;
    let mut minx: int32_t = 0;
    let mut miny: int32_t = 0;
    let mut maxx: int32_t = 0;
    let mut maxy: int32_t = 0;
    if angle < 1 as libc::c_int || angle >= 72 as libc::c_int {
        return;
    }
    if flip != 0 {
        idx += (*rotsprite).angles;
    ...
    RotatedPatch_CalculateDimensions(
        width,
        height,
        ca,
        sa,
    ...
    xcenter = xpivot * ((1 as libc::c_int) << 16 as libc::c_int);
    ycenter = ypivot * ((1 as libc::c_int) << 16 as libc::c_int);
    if xpivot != width / 2 as libc::c_int || ypivot != height / 2 as libc::c_int {
        newwidth *= 2 as libc::c_int;
        newheight *= 2 as libc::c_int;
    }
    minx = newwidth;
    miny = newheight;
    maxx = 0 as libc::c_int;
    maxy = 0 as libc::c_int;
    size = (newwidth * newheight) as size_t;
    if size == 0 {
        size = (width * height) as size_t;
    }
    rawdst = Z_CallocAlign(
        size.wrapping_mul(::core::mem::size_of::<uint16_t>() as libc::c_ulong),
        PU_STATIC as libc::c_int,
        0 as *mut libc::c_void,
        ::core::mem::size_of::<*mut libc::c_void>() as libc::c_ulong as int32_t,
    ) as *mut uint16_t;
    ...
    while dy < newheight {
        dx = 0 as libc::c_int;
        while dx < newwidth {
            x = (dx - newwidth / 2 as libc::c_int)
                * ((1 as libc::c_int) << 16 as libc::c_int);
            y = (dy - newheight / 2 as libc::c_int)
                * ((1 as libc::c_int) << 16 as libc::c_int);
            sx = FixedMul(x, ca) + FixedMul(y, sa) + xcenter;
            sy = -FixedMul(x, sa) + FixedMul(y, ca) + ycenter;
            sx >>= 16 as libc::c_int;
            sy >>= 16 as libc::c_int;
            if sx >= 0 as libc::c_int && sy >= 0 as libc::c_int && sx < width
                && sy < height
            {
                let mut input: *mut libc::c_void = Picture_GetPatchPixel(
                    patch,
                    PICFMT_PATCH,
                    sx,
                    sy,
    ...
    ox = newwidth / 2 as libc::c_int + (leftoffset - xpivot);
    oy = newheight / 2 as libc::c_int + ((*patch).topoffset as libc::c_int - ypivot);
    width = maxx - minx;
    height = maxy - miny;
    if (width * height) as libc::c_uint as size_t > size {
        let mut src: *mut uint16_t = 0 as *mut uint16_t;
        let mut dest: *mut uint16_t = 0 as *mut uint16_t;
        size = (width * height) as size_t;
        rawconv = Z_CallocAlign(
            size.wrapping_mul(::core::mem::size_of::<uint16_t>() as libc::c_ulong),
            PU_STATIC as libc::c_int,
            0 as *mut libc::c_void,
            ::core::mem::size_of::<*mut libc::c_void>() as libc::c_ulong as int32_t,
        ) as *mut uint16_t;
        src = &mut *rawdst.offset((miny * newwidth + minx) as isize) as *mut uint16_t;
        dest = rawconv;
        dy = height;
        loop {
            let fresh1 = dy;
            ...
            dest = dest.offset(width as isize);
            src = src.offset(newwidth as isize);
        }
        ox -= minx;
        oy -= miny;
        Z_Free(rawdst as *mut libc::c_void);
    } else {
        rawconv = rawdst;
        width = newwidth;
        height = newheight;
    }
    rotated = Picture_Convert(
        PICFMT_FLAT16,
        rawconv as *mut libc::c_void,
        PICFMT_PATCH,
    ...
    Z_ChangeTag(rotated as *mut libc::c_void, PU_PATCH_ROTATED as libc::c_int);
    Z_SetUser(
        rotated as *mut libc::c_void,
        &mut *((*rotsprite).patches).offset(idx as isize) as *mut *mut libc::c_void,
    );
    Z_Free(rawconv as *mut libc::c_void);
    (*rotated).leftoffset = ox as int16_t;
    (*rotated).topoffset = oy as int16_t;
}
