...
pub const CR_FAN: C2RustUnnamed_2 = 13;
pub const CR_DUSTDEVIL: C2RustUnnamed_2 = 12;
pub const CR_PTERABYTE: C2RustUnnamed_2 = 11;
pub const CR_ROLLOUT: C2RustUnnamed_2 = 10;
pub const CR_MINECART: C2RustUnnamed_2 = 9;
pub const CR_MACESPIN: C2RustUnnamed_2 = 8;
pub const CR_ROPEHANG: C2RustUnnamed_2 = 7;
pub const CR_ZOOMTUBE: C2RustUnnamed_2 = 6;
pub const CR_BRAKGOOP: C2RustUnnamed_2 = 5;
pub const CR_NIGHTSFALL: C2RustUnnamed_2 = 4;
pub const CR_NIGHTSMODE: C2RustUnnamed_2 = 3;
pub const CR_PLAYER: C2RustUnnamed_2 = 2;
pub const CR_GENERIC: C2RustUnnamed_2 = 1;
pub const CR_NONE: C2RustUnnamed_2 = 0;
...
#[no_mangle]
pub unsafe extern "C" fn P_MixUp(
    mut thing: *mut mobj_t,
    mut x: fixed_t,
    mut y: fixed_t,
    mut z: fixed_t,
    mut angle: angle_t,
    mut starpostx: int16_t,
    mut starposty: int16_t,
    mut starpostz: int16_t,
    mut starpostnum: int32_t,
    mut starposttime: tic_t,
    mut starpostangle: angle_t,
    mut starpostscale: fixed_t,
    mut drawangle: angle_t,
    mut flags2: int32_t,
) {
    let takeflags2: int32_t = MF2_TWOD as libc::c_int | MF2_OBJECTFLIP as libc::c_int;
    ...
    if !((*thing).player).is_null() {
        if (*thing).eflags as libc::c_int & MFE_VERTICALFLIP as libc::c_int != 0 {
            (*(*thing).player)
                .viewz = (*thing).z + (*thing).height - (*(*thing).player).viewheight;
        } else {
            (*(*thing).player).viewz = (*thing).z + (*(*thing).player).viewheight;
        }
        if ((*thing).tracer).is_null() {
            (*thing).reactiontime = 35 as libc::c_int / 2 as libc::c_int;
        }
        ...
        if splitscreen != 0 && camera2.chase != 0
            && (*thing).player
                == &mut *players.as_mut_ptr().offset(secondarydisplayplayer as isize)
                    as *mut player_t
        {
            P_ResetCamera((*thing).player, &mut camera2);
        } else if camera.chase != 0
            && (*thing).player
                == &mut *players.as_mut_ptr().offset(displayplayer as isize)
                    as *mut player_t
        {
            P_ResetCamera((*thing).player, &mut camera);
        }
        (*(*thing).player).cmomy = 0 as libc::c_int;
        (*(*thing).player).cmomx = (*(*thing).player).cmomy;
        (*(*thing).player).rmomy = 0 as libc::c_int;
        (*(*thing).player).rmomx = (*(*thing).player).rmomy;
        if ((*thing).tracer).is_null() {
            (*(*thing).player).speed = 0 as libc::c_int;
        }
    ...
    (*thing)
        .flags2 = (*thing).flags2 & !takeflags2 as uint32_t
        | (flags2 & takeflags2) as uint32_t;
}
#[no_mangle]
pub unsafe extern "C" fn P_Teleport(
    mut thing: *mut mobj_t,
    mut x: fixed_t,
    mut y: fixed_t,
    mut z: fixed_t,
    mut angle: angle_t,
    mut flash: boolean,
    mut dontstopmove: boolean,
) -> boolean {
    if P_SetOrigin(thing, x, y, z) == 0 {
        ...
        if dontstopmove == 0 {
            ...
            while p < 32 as libc::c_int {
                if playeringame[p as usize] != 0 && !(players[p as usize].mo).is_null()
                    ...
                    && (*players[p as usize].mo).tracer == thing
                {
                    players[p as usize]
                        .powers[pw_carry as libc::c_int
                        as usize] = CR_NONE as libc::c_int as uint16_t;
                    P_SetTarget2(
                        &mut (*(*players.as_mut_ptr().offset(p as isize)).mo).tracer,
                        0 as *mut mobj_t,
                    );
                    break;
                } else {
            ...
            (*(*thing).player)
                .drawangle = ((*(*thing).player).drawangle)
                .wrapping_add(angle.wrapping_sub((*thing).angle));
        }
        P_SetPlayerAngle((*thing).player, angle);
        if splitscreen != 0 && camera2.chase != 0
            && (*thing).player
