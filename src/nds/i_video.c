...
#define GPU_CMDBUF_SIZE		(1024 * 1024 * 16)
...
rendermode_t chosenrendermode = render_opengl;
...
#endif
	FOutVector *tmp = HWR_AllocVertexBuffer(iNumPts);
	memcpy(tmp, pOutVerts, sizeof(FOutVector) * iNumPts);

#ifdef DIAGNOSTIC
	if ((size_t)tmp < (size_t)geometryBuf ||
		(size_t) tmp >= (size_t)geometryBuf + (VCACHE_NUM_BUFFERS * MAX_NUM_VECTORS) * sizeof(FOutVector))
		NDS3D_driverPanic("Invalid geometry ptr passed to NDS3DVIDEO_DrawPolygon\n%p vs %p\n", pSurf, geometryBuf);
#endif

	size_t bufIndex = ((size_t)tmp - (size_t)geometryBuf)/sizeof(*tmp);

	queuePacket *packet = queueAllocPacketSafe();
	packet->type = CMD_TYPE_DRAW;
	packet->args.argsDraw.surfColor = pSurf ? pSurf->TintColor.rgba : 0xFFFFFFFF;
								...
								GrTextureFormat_t format, RGBA_t *bufOut, u8 downFactor)
{
	...
	// texFormat == GL_TEXFMT_AP_88
	else
	{
#ifdef DIAGNOSTIC
		if(format != GL_TEXFMT_AP_88)
...
void NDS3DVIDEO_UpdateTexture(GLMipmap_t *pTexInfo)
{
	...
	NDS3D_driverLog("NDS3DVIDEO_UpdateTexture\n");
	
	
	texData = pTexInfo->data;
	...
	height = pTexInfo->height;
	width = pTexInfo->width;
	...
	texFormat = pTexInfo->format;

	/* Downsample large palette-based textures */
	// if (!isNew3DS) 
	{
		if((texFormat == GL_TEXFMT_P_8 || texFormat == GL_TEXFMT_AP_88) &&
			(width > 32 && height > 32))
		{
			...
			NDS3D_driverLog("Texture dimensions (Resized): %i x %i\n", width, height);
	...
	switch(texFormat)
	{
		...
		*/
		...
		default:
			NDS3D_driverPanic("Unknown texture format! %i\n", texFormat);
			texPixelSize = 0;
			gpuFormat = 0;
	}
	
	
	texCurrent = texCacheAdd(pTexInfo);
	...
	*/
	...
	pTexInfo->downloaded = texCurrent;
}
void NDS3DVIDEO_SetTexture(GLMipmap_t *pTexInfo)
{
	...
	if(pTexInfo->downloaded)
	{
		...
		if((void *)pTexInfo->downloaded != texCurrent)
		{
			texCurrent = (void *)pTexInfo->downloaded;
		}
		
		NDS3D_driverLog("Activated downloaded texture %i\n", texCurrent);			
		return;
	}

	NDS3DVIDEO_UpdateTexture(pTexInfo);
	NDS3D_driverLog("downloaded: %i\n", pTexInfo->downloaded);
}

void NDS3DVIDEO_DeleteTexture(GLMipmap_t *pTexInfo)
{
	NDS3D_driverLog("NDS3DVIDEO_DeleteTexture\n");
...
void I_StartupGraphics(void)
{
	...
	HWD.pfnDrawIndexedTriangles = NDS3DVIDEO_Stub;
	HWD.pfnRenderSkyDome    = NDS3DVIDEO_Stub;
	HWD.pfnSetBlend         = NDS3DVIDEO_SetBlend;
	HWD.pfnClearBuffer      = NDS3DVIDEO_ClearBuffer;
	HWD.pfnSetTexture       = NDS3DVIDEO_SetTexture;
	HWD.pfnUpdateTexture    = NDS3DVIDEO_UpdateTexture;
	HWD.pfnDeleteTexture    = NDS3DVIDEO_DeleteTexture;
	...
	HWD.pfnDrawModel        = NDS3DVIDEO_Stub;
	HWD.pfnCreateModelVBOs  = NDS3DVIDEO_Stub;
	...
	HWD.pfnPostImgRedraw    = NDS3DVIDEO_Stub;
	HWD.pfnFlushScreenTextures                       = NDS3DVIDEO_FlushScreenTextures;
	HWD.pfnStartScreenWipe                           = NDS3DVIDEO_StartScreenWipe;
	HWD.pfnEndScreenWipe                             = NDS3DVIDEO_EndScreenWipe;
	HWD.pfnDoScreenWipe                              = NDS3DVIDEO_DoScreenWipe;
	...
	HWD.pfnUnSetShader				 = NDS3DVIDEO_Stub;
	HWD.pfnSetShader				 = NDS3DVIDEO_Stub;
	

	CV_RegisterVar(&cv_vidwait);

	bool ok = C3D_Init(GPU_CMDBUF_SIZE);//Ex(GPU_CMDBUF_SIZE, GPU_GXQUEUE_SIZE, true);
	if (!ok) {
