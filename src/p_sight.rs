...
extern "C" {
    ...
    static mut rejectmatrix: *mut uint8_t;
...
pub const POF_SPLAT: C2RustUnnamed_1 = 8192;
pub const POF_NOSPECIALS: C2RustUnnamed_1 = 4096;
pub const POF_ONESIDE: C2RustUnnamed_1 = 2048;
pub const POF_LDEXEC: C2RustUnnamed_1 = 1024;
pub const POF_PUSHABLESTOP: C2RustUnnamed_1 = 512;
pub const POF_INVERTPLANESONLY: C2RustUnnamed_1 = 256;
pub const POF_INVERTPLANES: C2RustUnnamed_1 = 128;
pub const POF_INVERT: C2RustUnnamed_1 = 64;
pub const POF_RENDERALL: C2RustUnnamed_1 = 56;
pub const POF_RENDERPLANES: C2RustUnnamed_1 = 48;
pub const POF_RENDERBOTTOM: C2RustUnnamed_1 = 32;
pub const POF_RENDERTOP: C2RustUnnamed_1 = 16;
pub const POF_RENDERSIDES: C2RustUnnamed_1 = 8;
pub const POF_TESTHEIGHT: C2RustUnnamed_1 = 4;
pub const POF_SOLID: C2RustUnnamed_1 = 3;
pub const POF_CLIPPLANES: C2RustUnnamed_1 = 2;
pub const POF_CLIPLINES: C2RustUnnamed_1 = 1;
...
#[repr(C)]
pub struct los_t {
    pub sightzstart: fixed_t,
    pub t2x: fixed_t,
    pub t2y: fixed_t,
    pub strace: divline_t,
...
static mut sightcounts: [int32_t; 2] = [0; 2];
unsafe extern "C" fn P_DivlineSide(
    mut x: fixed_t,
    mut y: fixed_t,
    ...
    if (*node).dx == 0 {
        ...
        if x <= (*node).x {
            return ((*node).dy > 0 as libc::c_int) as libc::c_int;
        }
        return ((*node).dy < 0 as libc::c_int) as libc::c_int;
    }
    if (*node).dy == 0 {
        ...
        if y <= (*node).y {
            return ((*node).dx < 0 as libc::c_int) as libc::c_int;
        }
        return ((*node).dx > 0 as libc::c_int) as libc::c_int;
    }
    dx = x - (*node).x;
    dy = y - (*node).y;
    left = ((*node).dy >> 16 as libc::c_int) * (dx >> 16 as libc::c_int);
    right = (dy >> 16 as libc::c_int) * ((*node).dx >> 16 as libc::c_int);
    if right < left {
...
unsafe extern "C" fn P_InterceptVector2(
...
unsafe extern "C" fn P_CrossSubsecPolyObj(
    ...
    while i < (*po).numLines {
        ...
        let mut v1: *const vertex_t = 0 as *const vertex_t;
        let mut v2: *const vertex_t = 0 as *const vertex_t;
        let mut frac: fixed_t = 0;
        let mut topslope: fixed_t = 0;
        let mut bottomslope: fixed_t = 0;
        if !((*line).validcount == validcount) {
            (*line).validcount = validcount;
            if !((*line).bbox[BOXLEFT as libc::c_int as usize]
                > (*los).bbox[BOXRIGHT as libc::c_int as usize]
                || (*line).bbox[BOXRIGHT as libc::c_int as usize]
                    < (*los).bbox[BOXLEFT as libc::c_int as usize]
                || (*line).bbox[BOXBOTTOM as libc::c_int as usize]
                    > (*los).bbox[BOXTOP as libc::c_int as usize]
                || (*line).bbox[BOXTOP as libc::c_int as usize]
                    < (*los).bbox[BOXBOTTOM as libc::c_int as usize])
            {
                v1 = (*line).v1;
                v2 = (*line).v2;
                if !(P_DivlineSide((*v1).x, (*v1).y, &mut (*los).strace)
                    == P_DivlineSide((*v2).x, (*v2).y, &mut (*los).strace))
                {
                    divl.x = (*v1).x;
                    divl.dx = (*v2).x - divl.x;
                    divl.y = (*v1).y;
                    divl.dy = (*v2).y - divl.y;
                    if !(P_DivlineSide((*los).strace.x, (*los).strace.y, &mut divl)
                        == P_DivlineSide((*los).t2x, (*los).t2y, &mut divl))
                    {
                        frac = P_InterceptVector2(&mut (*los).strace, &mut divl);
                        topslope = FixedDiv(
                            (*polysec).ceilingheight - (*los).sightzstart,
                            frac,
                        );
                        bottomslope = FixedDiv(
                            (*polysec).floorheight - (*los).sightzstart,
                            frac,
                        );
                        if topslope >= (*los).topslope
                            && bottomslope <= (*los).bottomslope
                        {
...
unsafe extern "C" fn P_CrossSubsector(mut num: size_t, mut los: *mut los_t) -> boolean {
    let mut seg: *mut seg_t = 0 as *mut seg_t;
    let mut count: int32_t = 0;
    let mut po: *mut polyobj_t = 0 as *mut polyobj_t;
    seg = segs.offset((*subsectors.offset(num as isize)).firstline as isize);
    po = (*subsectors.offset(num as isize)).polyList;
    if !po.is_null() {
        while !po.is_null() {
            if (*po).validcount != validcount {
                (*po).validcount = validcount;
                if P_CrossSubsecPolyObj(po, los) == 0 {
                    return false_0 as libc::c_int;
                }
            }
    ...
    count = (*subsectors.offset(num as isize)).numlines as int32_t;
    loop {
        count -= 1;
        if !(count >= 0 as libc::c_int) {
            break;
        }
        let mut line: *mut line_t = (*seg).linedef;
        let mut divl: divline_t = divline_t {
            x: 0,
            y: 0,
            dx: 0,
        ...
        let mut popentop: fixed_t = 0;
        let mut popenbottom: fixed_t = 0;
        let mut front: *const sector_t = 0 as *const sector_t;
        let mut back: *const sector_t = 0 as *const sector_t;
        let mut v1: *const vertex_t = 0 as *const vertex_t;
        let mut v2: *const vertex_t = 0 as *const vertex_t;
        let mut frac: fixed_t = 0;
        let mut frontf: fixed_t = 0;
        let mut backf: fixed_t = 0;
        let mut frontc: fixed_t = 0;
        let mut backc: fixed_t = 0;
        let mut fracx: fixed_t = 0;
        let mut fracy: fixed_t = 0;
        if !((*seg).glseg != 0) {
            if !((*line).validcount == validcount) {
                (*line).validcount = validcount;
                if !((*line).bbox[BOXLEFT as libc::c_int as usize]
                        ...
                        < (*los).bbox[BOXBOTTOM as libc::c_int as usize])
                {
                        ...
                        == P_DivlineSide((*v2).x, (*v2).y, &mut (*los).strace))
                    {
                        ...
                        if !(P_DivlineSide((*los).strace.x, (*los).strace.y, &mut divl)
                            == P_DivlineSide((*los).t2x, (*los).t2y, &mut divl))
                        {
                            if (*line).flags as libc::c_int & 4 as libc::c_int == 0 {
                                return false_0 as libc::c_int;
                            }
                            frac = P_InterceptVector2(&mut (*los).strace, &mut divl);
                            ...
                            if !(frontf == backf && frontc == backc
                                && ((*front).ffloors).is_null() as libc::c_int
                                    & ((*back).ffloors).is_null() as libc::c_int != 0)
                            {
                                popentop = if frontc < backc { frontc } else { backc };
                                popenbottom = if frontf > backf { frontf } else { backf };
                                if popenbottom >= popentop {
                                    return false_0 as libc::c_int;
                                }
                                if frontf != backf {
                                    let mut slope: fixed_t = FixedDiv(
                                        popenbottom - (*los).sightzstart,
                                        frac,
                                    );
                                ...
                                if frontc != backc {
                                    let mut slope_0: fixed_t = FixedDiv(
                                        popentop - (*los).sightzstart,
                                        frac,
                                    );
                                    if slope_0 < (*los).topslope {
                                        (*los).topslope = slope_0;
                                    }
                                }
...
unsafe extern "C" fn P_CrossBSPNode(
    mut bspnum: int32_t,
    mut los: *mut los_t,
) -> boolean {
    while bspnum & 0x8000 as libc::c_int == 0 {
        let mut bsp: *mut node_t = nodes.offset(bspnum as isize);
        let mut side: int32_t = P_DivlineSide(
            (*los).strace.x,
            (*los).strace.y,
            bsp as *mut divline_t,
        ) & 1 as libc::c_int;
        if side == P_DivlineSide((*los).t2x, (*los).t2y, bsp as *mut divline_t) {
            bspnum = (*bsp).children[side as usize] as int32_t;
        } else if P_CrossBSPNode((*bsp).children[side as usize] as int32_t, los) == 0 {
            return 0 as libc::c_int
        } else {
            bspnum = (*bsp).children[(side ^ 1 as libc::c_int) as usize] as int32_t;
        }
    }
    return P_CrossSubsector(
...
pub unsafe extern "C" fn P_CheckSight(
    ...
    let mut s1: *const sector_t = 0 as *const sector_t;
    let mut s2: *const sector_t = 0 as *const sector_t;
    let mut pnum: size_t = 0;
    let mut los: los_t = los_t {
        sightzstart: 0,
        t2x: 0,
        t2y: 0,
        strace: divline_t {
    ...
    if ((*t1).subsector).is_null() || ((*t2).subsector).is_null()
        || ((*(*t1).subsector).sector).is_null() || ((*(*t2).subsector).sector).is_null()
    {
        return false_0 as libc::c_int;
    }
    s1 = (*(*t1).subsector).sector;
    s2 = (*(*t2).subsector).sector;
    pnum = (s1.offset_from(sectors) as libc::c_long as size_t * numsectors)
        .wrapping_add(s2.offset_from(sectors) as libc::c_long as size_t);
    if !rejectmatrix.is_null() {
        if *rejectmatrix.offset((pnum >> 3 as libc::c_int) as isize) as libc::c_int
            & (1 as libc::c_int) << (pnum & 7 as libc::c_int as size_t) != 0
        {
            return false_0 as libc::c_int;
        }
    }
    if ((*(*t1).subsector).polyList).is_null() && (*t1).subsector == (*t2).subsector {
        return true_0 as libc::c_int;
    }
    sightcounts[1 as libc::c_int as usize] += 1;
    sightcounts[1 as libc::c_int as usize];
    validcount = validcount.wrapping_add(1);
    validcount;
    los.sightzstart = (*t1).z + (*t1).height - ((*t1).height >> 2 as libc::c_int);
    ...
    if (*t1).x > (*t2).x {
        los.bbox[BOXRIGHT as libc::c_int as usize] = (*t1).x;
        los.bbox[BOXLEFT as libc::c_int as usize] = (*t2).x;
    } else {
        los.bbox[BOXRIGHT as libc::c_int as usize] = (*t2).x;
        los.bbox[BOXLEFT as libc::c_int as usize] = (*t1).x;
    }
    if (*t1).y > (*t2).y {
        los.bbox[BOXTOP as libc::c_int as usize] = (*t1).y;
        los.bbox[BOXBOTTOM as libc::c_int as usize] = (*t2).y;
    } else {
        los.bbox[BOXTOP as libc::c_int as usize] = (*t2).y;
        los.bbox[BOXBOTTOM as libc::c_int as usize] = (*t1).y;
    }
    if s1 == s2 {
        let mut rover: *mut ffloor_t = 0 as *mut ffloor_t;
        let mut topz1: fixed_t = 0;
        let mut bottomz1: fixed_t = 0;
        let mut topz2: fixed_t = 0;
        let mut bottomz2: fixed_t = 0;
        rover = (*s1).ffloors;
        while !rover.is_null() {
            if !((*rover).fofflags as libc::c_uint
                & FOF_EXISTS as libc::c_int as libc::c_uint == 0
                        ...
                        as libc::c_uint != 0)
            {
                    ...
                    & FOF_SOLID as libc::c_int as libc::c_uint != 0)
                {
                            ...
                            & FOF_INVERTPLANES as libc::c_int as libc::c_uint != 0
                    {
                        ...
                        if los.sightzstart >= bottomz1
                            && (*t2).z + (*t2).height < bottomz2
                        {
                            return false_0 as libc::c_int;
                        }
                    }
    ...
    return P_CrossBSPNode(numnodes as int32_t - 1 as libc::c_int, &mut los);
}
