...
extern "C" {
    ...
    fn FV3_Dot(a_1: *const vector3_t, a_2: *const vector3_t) -> fixed_t;
    fn FV3_Divide(a_i: *mut vector3_t, a_c: fixed_t) -> *mut vector3_t;
    fn FV3_Mul(a_i: *mut vector3_t, a_c: fixed_t) -> *mut vector3_t;
    fn FV3_SubEx(
        a_i: *const vector3_t,
        a_c: *const vector3_t,
        a_o: *mut vector3_t,
    ...
    static mut blockmaplump: *mut int32_t;
    static mut blockmap: *mut int32_t;
    static mut bmaporgy: fixed_t;
    static mut bmaporgx: fixed_t;
    static mut mapcampointer: *mut camera_t;
    static mut tmy: fixed_t;
    static mut tmx: fixed_t;
    fn P_CameraFloorZ(
        mobj: *mut camera_t,
        sector: *mut sector_t,
    ...
    fn P_CreatePrecipSecNodeList(thing: *mut precipmobj_t, x: fixed_t, y: fixed_t);
    fn P_MobjFloorZ(
        mobj: *mut mobj_t,
        sector: *mut sector_t,
        boundsec: *mut sector_t,
    ...
    fn P_CreateSecNodeList(thing: *mut mobj_t, x: fixed_t, y: fixed_t);
...
pub unsafe extern "C" fn P_AproxDistance(mut dx: fixed_t, mut dy: fixed_t) -> fixed_t {
    ...
    dy = abs(dy);
    if dx < dy {
        return dx + dy - (dx >> 1 as libc::c_int);
    }
    return dx + dy - (dy >> 1 as libc::c_int);
}
#[no_mangle]
pub unsafe extern "C" fn P_ClosestPointOnLine(
    mut x: fixed_t,
    mut y: fixed_t,
    mut line: *mut line_t,
    mut result: *mut vertex_t,
) {
    let mut startx: fixed_t = (*(*line).v1).x;
    let mut starty: fixed_t = (*(*line).v1).y;
    let mut dx: fixed_t = (*line).dx;
    let mut dy: fixed_t = (*line).dy;
    let mut cx: fixed_t = 0;
    let mut cy: fixed_t = 0;
    let mut vx: fixed_t = 0;
    let mut vy: fixed_t = 0;
    let mut magnitude: fixed_t = 0;
    let mut t: fixed_t = 0;
    cx = x - startx;
    cy = y - starty;
    vx = dx;
...
#[no_mangle]
pub unsafe extern "C" fn P_ClosestPointOnLine3D(
    mut p: *const vector3_t,
    mut Line: *const vector3_t,
    mut result: *mut vector3_t,
) {
    let mut v1: *const vector3_t = &*Line.offset(0 as libc::c_int as isize)
        as *const vector3_t;
    let mut v2: *const vector3_t = &*Line.offset(1 as libc::c_int as isize)
        as *const vector3_t;
    let mut c: vector3_t = vector3_t { x: 0, y: 0, z: 0 };
    let mut V: vector3_t = vector3_t { x: 0, y: 0, z: 0 };
    let mut n: vector3_t = vector3_t { x: 0, y: 0, z: 0 };
    let mut t: fixed_t = 0;
    let mut d: fixed_t = 0;
    FV3_SubEx(v2, v1, &mut V);
    FV3_SubEx(p, v1, &mut c);
    d = R_PointToDist2(
        0 as libc::c_int,
        (*v2).z,
        R_PointToDist2((*v2).x, (*v2).y, (*v1).x, (*v1).y),
        (*v1).z,
    );
    FV3_Copy(&mut n, &mut V);
    FV3_Divide(&mut n, d);
    t = FV3_Dot(&mut n, &mut c);
...
pub unsafe extern "C" fn P_PointOnLineSide(
    ...
    let mut v1: *const vertex_t = (*line).v1;
    ...
    if (*line).dx == 0 {
        if x <= (*v1).x {
            return ((*line).dy > 0 as libc::c_int) as libc::c_int;
        }
        return ((*line).dy < 0 as libc::c_int) as libc::c_int;
    }
    if (*line).dy == 0 {
        if y <= (*v1).y {
            return ((*line).dx < 0 as libc::c_int) as libc::c_int;
        }
        return ((*line).dx > 0 as libc::c_int) as libc::c_int;
    }
    dx = x - (*v1).x;
    dy = y - (*v1).y;
    left = FixedMul((*line).dy >> 16 as libc::c_int, dx);
    right = FixedMul(dy, (*line).dx >> 16 as libc::c_int);
    if right < left {
...
pub unsafe extern "C" fn P_BoxOnLineSide(
    ...
    let mut p1: int32_t = 0;
    let mut p2: int32_t = 0;
    match (*ld).slopetype as libc::c_uint {
        0 => {
            p1 = (*tmbox.offset(BOXTOP as libc::c_int as isize) > (*(*ld).v1).y)
                as libc::c_int;
            p2 = (*tmbox.offset(BOXBOTTOM as libc::c_int as isize) > (*(*ld).v1).y)
                as libc::c_int;
            if (*ld).dx < 0 as libc::c_int {
                p1 ^= 1 as libc::c_int;
                p2 ^= 1 as libc::c_int;
            }
        }
        1 => {
            p1 = (*tmbox.offset(BOXRIGHT as libc::c_int as isize) < (*(*ld).v1).x)
                as libc::c_int;
            p2 = (*tmbox.offset(BOXLEFT as libc::c_int as isize) < (*(*ld).v1).x)
                as libc::c_int;
            if (*ld).dy < 0 as libc::c_int {
                p1 ^= 1 as libc::c_int;
                p2 ^= 1 as libc::c_int;
            }
        }
        2 => {
            p1 = P_PointOnLineSide(
                *tmbox.offset(BOXLEFT as libc::c_int as isize),
                *tmbox.offset(BOXTOP as libc::c_int as isize),
                ld,
            );
            p2 = P_PointOnLineSide(
                *tmbox.offset(BOXRIGHT as libc::c_int as isize),
                *tmbox.offset(BOXBOTTOM as libc::c_int as isize),
                ld,
            );
        }
        3 => {
        ...
        _ => {
            I_Error(
                b"P_BoxOnLineSide: unknown slopetype %d\n\0" as *const u8
                    as *const libc::c_char,
                (*ld).slopetype as libc::c_uint,
            );
        }
    }
    if p1 == p2 {
...
unsafe extern "C" fn P_PointOnDivlineSide(
    mut x: fixed_t,
    mut y: fixed_t,
    ...
    dx = x - (*line).x;
    dy = y - (*line).y;
    if ((*line).dy ^ (*line).dx ^ dx ^ dy) as libc::c_uint & 0x80000000 as libc::c_uint
        != 0
    {
        if ((*line).dy ^ dx) as libc::c_uint & 0x80000000 as libc::c_uint != 0 {
            return 1 as libc::c_int;
        }
        return 0 as libc::c_int;
    }
    left = FixedMul((*line).dy >> 8 as libc::c_int, dx >> 8 as libc::c_int);
    right = FixedMul(dy >> 8 as libc::c_int, (*line).dx >> 8 as libc::c_int);
    if right < left {
        return 0 as libc::c_int;
    }
    return 1 as libc::c_int;
...
pub unsafe extern "C" fn P_InterceptVector(
    ...
    den = FixedMul((*v1).dy >> 8 as libc::c_int, (*v2).dx)
        - FixedMul((*v1).dx >> 8 as libc::c_int, (*v2).dy);
    if den == 0 {
        return 0 as libc::c_int;
    }
    num = FixedMul((*v1).x - (*v2).x >> 8 as libc::c_int, (*v1).dy)
        + FixedMul((*v2).y - (*v1).y >> 8 as libc::c_int, (*v1).dx);
...
#[no_mangle]
pub static mut highceiling: fixed_t = 0;
#[no_mangle]
pub static mut opentopslope: *mut pslope_t = 0 as *const pslope_t as *mut pslope_t;
#[no_mangle]
pub static mut openbottomslope: *mut pslope_t = 0 as *const pslope_t as *mut pslope_t;
#[no_mangle]
pub static mut openfloorrover: *mut ffloor_t = 0 as *const ffloor_t as *mut ffloor_t;
#[no_mangle]
pub static mut openceilingrover: *mut ffloor_t = 0 as *const ffloor_t as *mut ffloor_t;
#[no_mangle]
pub unsafe extern "C" fn P_CameraLineOpening(mut linedef: *mut line_t) {
    let mut front: *mut sector_t = 0 as *mut sector_t;
    let mut back: *mut sector_t = 0 as *mut sector_t;
    let mut frontfloor: fixed_t = 0;
    let mut frontceiling: fixed_t = 0;
    let mut backfloor: fixed_t = 0;
    let mut backceiling: fixed_t = 0;
    if (*linedef).sidenum[1 as libc::c_int as usize] as libc::c_int
        == 0xffff as libc::c_int
    {
        openrange = 0 as libc::c_int;
        return;
    }
    front = (*linedef).frontsector;
    back = (*linedef).backsector;
    if (*front).camsec >= 0 as libc::c_int {
        frontfloor = P_GetSectorFloorZAt(
            &mut *sectors.offset((*front).camsec as isize),
            camera.x,
            camera.y,
        );
    ...
    } else if (*front).heightsec >= 0 as libc::c_int {
        frontfloor = P_GetSectorFloorZAt(
            &mut *sectors.offset((*front).heightsec as isize),
            camera.x,
            camera.y,
        );
        frontceiling = P_GetSectorCeilingZAt(
    ...
    if (*back).camsec >= 0 as libc::c_int {
        backfloor = P_GetSectorFloorZAt(
            &mut *sectors.offset((*back).camsec as isize),
            camera.x,
            camera.y,
        );
    ...
    } else if (*back).heightsec >= 0 as libc::c_int {
        backfloor = P_GetSectorFloorZAt(
            &mut *sectors.offset((*back).heightsec as isize),
            camera.x,
            camera.y,
        );
        backceiling = P_GetSectorCeilingZAt(
    ...
    let mut thingtop: fixed_t = (*mapcampointer).z + (*mapcampointer).height;
            ...
            while !rover.is_null() {
                        ...
                        & MSF_NOCLIPCAMERA as libc::c_int as libc::c_uint != 0)
                {
                    ...
                    delta1 = abs(
                        (*mapcampointer).z
                            - (bottomheight_0
                                + (topheight_0 - bottomheight_0) / 2 as libc::c_int),
                    );
                    delta2 = abs(
                        thingtop
                            - (bottomheight_0
                    ...
                    if bottomheight_0 < opentop && delta1 >= delta2 {
                        opentop = bottomheight_0;
                    } else if bottomheight_0 < highceiling && delta1 >= delta2 {
                        highceiling = bottomheight_0;
                    }
                    if topheight_0 > openbottom && delta1 < delta2 {
                        openbottom = topheight_0;
                    } else if topheight_0 > lowfloor && delta1 < delta2 {
                        lowfloor = topheight_0;
                    }
                }
                rover = (*rover).next;
            }
...
#[no_mangle]
pub unsafe extern "C" fn P_LineOpening(mut linedef: *mut line_t, mut mobj: *mut mobj_t) {
    let mut front: *mut sector_t = 0 as *mut sector_t;
    let mut back: *mut sector_t = 0 as *mut sector_t;
    if (*linedef).sidenum[1 as libc::c_int as usize] as libc::c_int
        == 0xffff as libc::c_int
    ...
    front = (*linedef).frontsector;
    back = (*linedef).backsector;
    openceilingrover = 0 as *mut ffloor_t;
    openfloorrover = openceilingrover;
    if !((*linedef).polyobj).is_null() {
        opentop = 2147483647 as libc::c_int;
        openbottom = -(2147483647 as libc::c_int) - 1 as libc::c_int;
        highceiling = -(2147483647 as libc::c_int) - 1 as libc::c_int;
        lowfloor = 2147483647 as libc::c_int;
        openbottomslope = 0 as *mut pslope_t;
        opentopslope = openbottomslope;
    } else {
        let mut frontheight: fixed_t = 0;
        let mut backheight: fixed_t = 0;
        frontheight = P_MobjCeilingZ(
            mobj,
            front,
            0 as *mut sector_t,
            tmx,
    ...
    if !mobj.is_null() {
        ...
        if (*linedef).flags as libc::c_int & 512 as libc::c_int != 0
            && ((*linedef).polyobj).is_null()
        {
            let mut side: *mut side_t = &mut *sides
                .offset(
                    *((*linedef).sidenum).as_mut_ptr().offset(0 as libc::c_int as isize)
                        as isize,
                ) as *mut side_t;
            let mut textop: fixed_t = 0;
            let mut texbottom: fixed_t = 0;
            let mut texheight: fixed_t = 0;
            let mut texmid: fixed_t = 0;
            let mut delta1: fixed_t = 0;
            let mut delta2: fixed_t = 0;
            let mut texnum: int32_t = R_GetTextureNum((*side).midtexture);
            if texnum != 0 {
                texheight = ((**textures.offset(texnum as isize)).height as libc::c_int)
                    << 16 as libc::c_int;
                if (*linedef).flags as libc::c_int & 1024 as libc::c_int != 0
                    && (*side).repeatcnt == 0
                {
                    ...
                    textop = texbottom
                        + texheight
                            * ((*side).repeatcnt as libc::c_int + 1 as libc::c_int);
                } else {
                    textop = opentop + (*side).rowoffset + (*side).offsety_mid;
                    texbottom = textop
                        - texheight
                ...
                texmid = texbottom + (textop - texbottom) / 2 as libc::c_int;
                ...
                } else if openbottom < textop {
                    openbottom = textop;
                }
            }
        }
        if !((*linedef).polyobj).is_null() {
            if (*(*linedef).polyobj).flags & POF_TESTHEIGHT as libc::c_int != 0 {
                let mut polysec: *const sector_t = (*linedef).backsector;
                let mut polytop: fixed_t = 0;
                let mut polybottom: fixed_t = 0;
                let mut delta1_0: fixed_t = 0;
                let mut delta2_0: fixed_t = 0;
                if (*(*linedef).polyobj).flags & POF_CLIPPLANES as libc::c_int != 0 {
                ...
                delta1_0 = abs(
                    (*mobj).z - (polybottom + (polytop - polybottom) / 2 as libc::c_int),
                );
                delta2_0 = abs(
                    thingtop - (polybottom + (polytop - polybottom) / 2 as libc::c_int),
                );
                if polybottom < opentop && delta1_0 >= delta2_0 {
                    opentop = polybottom;
                } else if polybottom < highceiling && delta1_0 >= delta2_0 {
                    highceiling = polybottom;
                }
                if polytop > openbottom && delta1_0 < delta2_0 {
                    openbottom = polytop;
                } else if polytop > lowfloor && delta1_0 < delta2_0 {
                    lowfloor = polytop;
                }
            }
        } else if !((*front).ffloors).is_null() || !((*back).ffloors).is_null() {
            let mut rover: *mut ffloor_t = 0 as *mut ffloor_t;
            let mut delta1_1: fixed_t = 0;
            let mut delta2_1: fixed_t = 0;
                    ...
                    & FOF_EXISTS as libc::c_int as libc::c_uint == 0)
                {
                    if !((*mobj).player).is_null()
                        && (P_CheckSolidLava(rover) != 0
                            || P_CanRunOnWater((*mobj).player, rover) != 0)
                    {
                        current_block_109 = 11796148217846552555;
                        ...
                        || (*rover).fofflags as libc::c_uint
                            ...
                            && ((*mobj).player).is_null())
                    {
                        current_block_109 = 13853033528615664019;
                    } else {
                        current_block_109 = 11796148217846552555;
                    }
                    match current_block_109 {
                        ...
                        _ => {
                            ...
                            if delta1_1 >= delta2_1
                                && (*rover).fofflags as libc::c_uint
                                    & FOF_INTANGIBLEFLATS as libc::c_int as libc::c_uint
                    ...
                    & FOF_EXISTS as libc::c_int as libc::c_uint == 0)
                {
                    if !((*mobj).player).is_null()
                        && (P_CheckSolidLava(rover) != 0
                            || P_CanRunOnWater((*mobj).player, rover) != 0)
                    {
                        current_block_134 = 2945622622075328793;
                    } else if !((*rover).fofflags as libc::c_uint
                        & FOF_BLOCKPLAYER as libc::c_int as libc::c_uint != 0
                        && !((*mobj).player).is_null()
                        || (*rover).fofflags as libc::c_uint
                            ...
                            && ((*mobj).player).is_null())
                    {
                        current_block_134 = 18137396335907573669;
                    } else {
                        current_block_134 = 2945622622075328793;
                    }
                        ...
                        _ => {
                            ...
                            if delta1_1 >= delta2_1
                                && (*rover).fofflags as libc::c_uint
                                    ...
                                    != FOF_PLATFORM as libc::c_int as libc::c_uint
                            {
                                if bottomheight_0 < opentop {
                                    opentop = bottomheight_0;
                                    opentopslope = *(*rover).b_slope;
                                    openceilingrover = rover;
                                } else if bottomheight_0 < highceiling {
                                    highceiling = bottomheight_0;
                                }
                            }
                            if delta1_1 < delta2_1
                                && (*rover).fofflags as libc::c_uint
                                    ...
                                    != FOF_REVERSEPLATFORM as libc::c_int as libc::c_uint
                            {
                                if topheight_0 > openbottom {
                                    openbottom = topheight_0;
                                    openbottomslope = *(*rover).t_slope;
                                    openfloorrover = rover;
                                } else if topheight_0 > lowfloor {
                                    lowfloor = topheight_0;
                                }
                            }
                        }
...
pub unsafe extern "C" fn P_UnsetThingPosition(mut thing: *mut mobj_t) {
    if (*thing).flags & MF_NOSECTOR as libc::c_int as uint32_t == 0 {
        let mut sprev: *mut *mut mobj_t = (*thing).sprev;
        let mut snext: *mut mobj_t = (*thing).snext;
        *sprev = snext;
        if !(*sprev).is_null() {
            (*snext).sprev = sprev;
        }
        sector_list = (*thing).touching_sectorlist;
        (*thing).touching_sectorlist = 0 as *mut msecnode_s;
    }
    if (*thing).flags & MF_NOBLOCKMAP as libc::c_int as uint32_t == 0 {
        let mut bnext: *mut mobj_t = 0 as *mut mobj_t;
        let mut bprev: *mut *mut mobj_t = (*thing).bprev;
        if !bprev.is_null()
            && {
                bnext = (*thing).bnext;
                *bprev = bnext;
...
#[no_mangle]
pub unsafe extern "C" fn P_UnsetPrecipThingPosition(mut thing: *mut precipmobj_t) {
    let mut sprev: *mut *mut precipmobj_t = (*thing).sprev;
    let mut snext: *mut precipmobj_t = (*thing).snext;
    *sprev = snext;
    if !(*sprev).is_null() {
        (*snext).sprev = sprev;
    }
    precipsector_list = (*thing).touching_sectorlist;
    (*thing).touching_sectorlist = 0 as *mut mprecipsecnode_s;
}
#[no_mangle]
pub unsafe extern "C" fn P_SetThingPosition(mut thing: *mut mobj_t) {
    let mut ss: *mut subsector_t = 0 as *mut subsector_t;
    let mut oldsec: *mut sector_t = 0 as *mut sector_t;
    let mut tfloorz: fixed_t = 0;
    let mut tceilz: fixed_t = 0;
    if !((*thing).player).is_null() && (*thing).z <= (*thing).floorz
        && !((*thing).subsector).is_null()
    {
    ...
    ss = (*thing).subsector;
    if (*thing).flags & MF_NOSECTOR as libc::c_int as uint32_t == 0 {
        let mut link: *mut *mut mobj_t = &mut (*(*ss).sector).thinglist;
        let mut snext: *mut mobj_t = *link;
        (*thing).snext = snext;
        if !((*thing).snext).is_null() {
            (*snext).sprev = &mut (*thing).snext;
        }
        (*thing).sprev = link;
        *link = thing;
    ...
    if (*thing).flags & MF_NOBLOCKMAP as libc::c_int as uint32_t == 0 {
        let blockx: int32_t = (((*thing).x - bmaporgx) as libc::c_uint
            >> 16 as libc::c_int + 7 as libc::c_int) as int32_t;
        let blocky: int32_t = (((*thing).y - bmaporgy) as libc::c_uint
            >> 16 as libc::c_int + 7 as libc::c_int) as int32_t;
        if blockx >= 0 as libc::c_int && blockx < bmapwidth && blocky >= 0 as libc::c_int
            && blocky < bmapheight
        {
            let mut link_0: *mut *mut mobj_t = &mut *blocklinks
                .offset((blocky * bmapwidth + blockx) as isize) as *mut *mut mobj_t;
            let mut bnext: *mut mobj_t = *link_0;
            (*thing).bnext = bnext;
            if !((*thing).bnext).is_null() {
                (*bnext).bprev = &mut (*thing).bnext;
            }
            (*thing).bprev = link_0;
            *link_0 = thing;
        } else {
            (*thing).bnext = 0 as *mut mobj_s;
            (*thing).bprev = 0 as *mut *mut mobj_s;
        }
    }
    if !((*thing).player).is_null() && !oldsec.is_null()
        && !((*thing).subsector).is_null() && oldsec != (*(*thing).subsector).sector
    {
        tfloorz = P_MobjFloorZ(
            thing,
            (*ss).sector,
            0 as *mut sector_t,
            (*thing).x,
...
#[no_mangle]
pub unsafe extern "C" fn P_SetUnderlayPosition(mut thing: *mut mobj_t) {
    let mut ss: *mut subsector_t = 0 as *mut subsector_t;
    let mut link: *mut *mut mobj_t = 0 as *mut *mut mobj_t;
    let mut lend: *mut mobj_t = 0 as *mut mobj_t;
    (*thing).subsector = R_PointInSubsector((*thing).x, (*thing).y);
    ss = (*thing).subsector;
    link = &mut (*(*ss).sector).thinglist;
    lend = *link;
    while !lend.is_null() && !((*lend).snext).is_null() {
        lend = (*lend).snext;
    }
    (*thing).snext = 0 as *mut mobj_s;
    if lend.is_null() {
        (*thing).sprev = link;
        *link = thing;
    } else {
        (*thing).sprev = &mut (*lend).snext;
        (*lend).snext = thing;
    }
    P_CreateSecNodeList(thing, (*thing).x, (*thing).y);
    (*thing).touching_sectorlist = sector_list;
...
#[no_mangle]
pub unsafe extern "C" fn P_SetPrecipitationThingPosition(mut thing: *mut precipmobj_t) {
    (*thing).subsector = R_PointInSubsector((*thing).x, (*thing).y);
    let mut ss: *mut subsector_t = (*thing).subsector;
    let mut link: *mut *mut precipmobj_t = &mut (*(*ss).sector).preciplist;
    let mut snext: *mut precipmobj_t = *link;
    (*thing).snext = snext;
    if !((*thing).snext).is_null() {
        (*snext).sprev = &mut (*thing).snext;
    }
    ...
    *link = thing;
    ...
    (*thing).touching_sectorlist = precipsector_list;
...
pub unsafe extern "C" fn P_BlockLinesIterator(
    ...
    let mut list: *const int32_t = 0 as *const int32_t;
    let mut plink: *mut polymaplink_t = 0 as *mut polymaplink_t;
    let mut ld: *mut line_t = 0 as *mut line_t;
    if x < 0 as libc::c_int || y < 0 as libc::c_int || x >= bmapwidth || y >= bmapheight
    {
    ...
    while !plink.is_null() {
        ...
        if (*po).validcount != validcount {
            ...
            while i < (*po).numLines {
                if !((**((*po).lines).offset(i as isize)).validcount == validcount) {
                    (**((*po).lines).offset(i as isize)).validcount = validcount;
                    if func
                        .expect(
                            "non-null function pointer",
                        )(*((*po).lines).offset(i as isize)) == 0
                    {
    ...
    offset = *blockmap.offset(offset as isize);
    list = blockmaplump.offset(offset as isize).offset(1 as libc::c_int as isize);
    while *list != -(1 as libc::c_int) {
        ld = &mut *lines.offset(*list as isize) as *mut line_t;
...
pub unsafe extern "C" fn P_BlockThingsIterator(
    ...
    while !mobj.is_null() {
        P_SetTarget2(&mut bnext, (*mobj).bnext);
        if func.expect("non-null function pointer")(mobj) == 0 {
            P_SetTarget2(&mut bnext, 0 as *mut mobj_t);
            return false_0 as libc::c_int;
        }
        if P_MobjWasRemoved(tmthing) != 0
            || !bnext.is_null() && P_MobjWasRemoved(bnext) != 0
        {
            P_SetTarget2(&mut bnext, 0 as *mut mobj_t);
            return true_0 as libc::c_int;
        }
...
static mut intercepts: *mut intercept_t = 0 as *const intercept_t as *mut intercept_t;
static mut intercept_p: *mut intercept_t = 0 as *const intercept_t as *mut intercept_t;
#[no_mangle]
...
static mut earlyout: boolean = 0;
unsafe extern "C" fn P_CheckIntercepts() {
    static mut max_intercepts: size_t = 0 as libc::c_int as size_t;
    let mut count: size_t = intercept_p.offset_from(intercepts) as libc::c_long
        as size_t;
    if max_intercepts <= count {
        if max_intercepts == 0 {
            max_intercepts = 128 as libc::c_int as size_t;
        } else {
            max_intercepts = max_intercepts * 2 as libc::c_int as size_t;
        }
        intercepts = Z_ReallocAlign(
            intercepts as *mut libc::c_void,
            (::core::mem::size_of::<intercept_t>() as libc::c_ulong)
                .wrapping_mul(max_intercepts),
            PU_STATIC as libc::c_int,
            0 as *mut libc::c_void,
            ::core::mem::size_of::<*mut libc::c_void>() as libc::c_ulong as int32_t,
        ) as *mut intercept_t;
        intercept_p = intercepts.offset(count as isize);
    }
}
unsafe extern "C" fn PIT_AddLineIntercepts(mut ld: *mut line_t) -> boolean {
    let mut s1: int32_t = 0;
    let mut s2: int32_t = 0;
    let mut frac: fixed_t = 0;
    ...
    if trace.dx > ((1 as libc::c_int) << 16 as libc::c_int) * 16 as libc::c_int
        || trace.dy > ((1 as libc::c_int) << 16 as libc::c_int) * 16 as libc::c_int
        || trace.dx < -((1 as libc::c_int) << 16 as libc::c_int) * 16 as libc::c_int
        || trace.dy < -((1 as libc::c_int) << 16 as libc::c_int) * 16 as libc::c_int
    {
        s1 = P_PointOnDivlineSide((*(*ld).v1).x, (*(*ld).v1).y, &mut trace);
        s2 = P_PointOnDivlineSide((*(*ld).v2).x, (*(*ld).v2).y, &mut trace);
    } else {
    ...
    P_MakeDivline(ld, &mut dl);
    frac = P_InterceptVector(&mut trace, &mut dl);
    if frac < 0 as libc::c_int {
        return true_0 as libc::c_int;
    }
    if earlyout != 0 && frac < (1 as libc::c_int) << 16 as libc::c_int
        && ((*ld).backsector).is_null()
    {
        return false_0 as libc::c_int;
    }
    P_CheckIntercepts();
    (*intercept_p).frac = frac;
    (*intercept_p).isaline = true_0 as libc::c_int;
    (*intercept_p).d.line = ld;
    intercept_p = intercept_p.offset(1);
    intercept_p;
    return true_0 as libc::c_int;
}
unsafe extern "C" fn PIT_AddThingIntercepts(mut thing: *mut mobj_t) -> boolean {
    let mut px1: fixed_t = 0;
    let mut py1: fixed_t = 0;
    let mut px2: fixed_t = 0;
    let mut py2: fixed_t = 0;
    let mut frac: fixed_t = 0;
    let mut s1: int32_t = 0;
    let mut s2: int32_t = 0;
    ...
    tracepositive = (trace.dx ^ trace.dy > 0 as libc::c_int) as libc::c_int;
    if tracepositive != 0 {
    ...
    s1 = P_PointOnDivlineSide(px1, py1, &mut trace);
    s2 = P_PointOnDivlineSide(px2, py2, &mut trace);
    if s1 == s2 {
        return true_0 as libc::c_int;
    }
    dl.x = px1;
    ...
    P_CheckIntercepts();
    (*intercept_p).frac = frac;
    (*intercept_p).isaline = false_0 as libc::c_int;
    (*intercept_p).d.thing = thing;
    intercept_p = intercept_p.offset(1);
    intercept_p;
    return true_0 as libc::c_int;
}
unsafe extern "C" fn P_TraverseIntercepts(
    mut func: traverser_t,
    mut maxfrac: fixed_t,
) -> boolean {
    let mut count: size_t = 0;
    let mut dist: fixed_t = 0;
    let mut scan: *mut intercept_t = 0 as *mut intercept_t;
    let mut in_0: *mut intercept_t = 0 as *mut intercept_t;
    count = intercept_p.offset_from(intercepts) as libc::c_long as size_t;
    loop {
        let fresh0 = count;
        ...
        (*in_0).frac = 2147483647 as libc::c_int;
    }
    return true_0 as libc::c_int;
}
#[no_mangle]
pub unsafe extern "C" fn P_PathTraverse(
    ...
    let mut mapx: int32_t = 0;
    let mut mapy: int32_t = 0;
    let mut mapxstep: int32_t = 0;
    let mut mapystep: int32_t = 0;
    let mut count: int32_t = 0;
    earlyout = flags & 4 as libc::c_int;
    ...
    {
        px1 += (1 as libc::c_int) << 16 as libc::c_int;
    }
    if py1 - bmaporgy
        & 128 as libc::c_int * ((1 as libc::c_int) << 16 as libc::c_int)
            - 1 as libc::c_int == 0 as libc::c_int
    {
        py1 += (1 as libc::c_int) << 16 as libc::c_int;
    }
    trace.x = px1;
    trace.y = py1;
    trace.dx = px2 - px1;
    ...
    px1 -= bmaporgx;
    py1 -= bmaporgy;
    xt1 = (px1 as libc::c_uint >> 16 as libc::c_int + 7 as libc::c_int) as fixed_t;
    yt1 = (py1 as libc::c_uint >> 16 as libc::c_int + 7 as libc::c_int) as fixed_t;
    px2 -= bmaporgx;
    py2 -= bmaporgy;
    xt2 = (px2 as libc::c_uint >> 16 as libc::c_int + 7 as libc::c_int) as fixed_t;
    yt2 = (py2 as libc::c_uint >> 16 as libc::c_int + 7 as libc::c_int) as fixed_t;
    if xt2 > xt1 {
        mapxstep = 1 as libc::c_int;
        partial = ((1 as libc::c_int) << 16 as libc::c_int)
            - (px1 >> 16 as libc::c_int + 7 as libc::c_int - 16 as libc::c_int
                & ((1 as libc::c_int) << 16 as libc::c_int) - 1 as libc::c_int);
        ystep = FixedDiv(py2 - py1, abs(px2 - px1));
    } else if xt2 < xt1 {
        mapxstep = -(1 as libc::c_int);
        partial = px1 >> 16 as libc::c_int + 7 as libc::c_int - 16 as libc::c_int
            & ((1 as libc::c_int) << 16 as libc::c_int) - 1 as libc::c_int;
        ystep = FixedDiv(py2 - py1, abs(px2 - px1));
    } else {
        mapxstep = 0 as libc::c_int;
        partial = (1 as libc::c_int) << 16 as libc::c_int;
        ystep = 256 as libc::c_int * ((1 as libc::c_int) << 16 as libc::c_int);
    }
    yintercept = (py1 >> 16 as libc::c_int + 7 as libc::c_int - 16 as libc::c_int)
        + FixedMul(partial, ystep);
    if yt2 > yt1 {
        mapystep = 1 as libc::c_int;
        partial = ((1 as libc::c_int) << 16 as libc::c_int)
            - (py1 >> 16 as libc::c_int + 7 as libc::c_int - 16 as libc::c_int
                & ((1 as libc::c_int) << 16 as libc::c_int) - 1 as libc::c_int);
        xstep = FixedDiv(px2 - px1, abs(py2 - py1));
    } else if yt2 < yt1 {
        mapystep = -(1 as libc::c_int);
        partial = py1 >> 16 as libc::c_int + 7 as libc::c_int - 16 as libc::c_int
            & ((1 as libc::c_int) << 16 as libc::c_int) - 1 as libc::c_int;
        xstep = FixedDiv(px2 - px1, abs(py2 - py1));
    } else {
        mapystep = 0 as libc::c_int;
        partial = (1 as libc::c_int) << 16 as libc::c_int;
        xstep = 256 as libc::c_int * ((1 as libc::c_int) << 16 as libc::c_int);
    }
    xintercept = (px1 >> 16 as libc::c_int + 7 as libc::c_int - 16 as libc::c_int)
        + FixedMul(partial, xstep);
    mapx = xt1;
    mapy = yt1;
    count = 0 as libc::c_int;
    while count < 64 as libc::c_int {
        ...
        if yintercept >> 16 as libc::c_int == mapy {
            yintercept += ystep;
            mapx += mapxstep;
        } else if xintercept >> 16 as libc::c_int == mapx {
            xintercept += xstep;
            mapy += mapystep;
        }
        count += 1;
    ...
    return P_TraverseIntercepts(trav, (1 as libc::c_int) << 16 as libc::c_int);
}
